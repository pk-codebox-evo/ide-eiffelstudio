		...
		internal_execute is
				...
				do
					if not eiffel_project.error_occurred then --should grey out on this
							list :=  eiffel_universe.classes_with_name ("I18N_LOCALE")
							if list.count > 0 then
								...
								if translator.compiled then
									...
									translation_function :=	compiled_translator.feature_named ("translate").rout_id_set
									plural_form_translation_function := compiled_translator.feature_named ("translate_plural").rout_id_set
