...
feature -- Processing
	...
	process_feature_as (l_as: FEATURE_AS) is
		do
			current_feature := current_class.feature_named (l_as.feature_name)
			Precursor (l_as)
...
feature -- Helper
	...
	nested_as_to_nested_expr_as (a_nested_as: NESTED_AS): NESTED_EXPR_AS is
		...
		do
			...
			if sub_n /= Void then
				...
				Result := factory.new_nested_expr_as (new_expr,a_nested_as.message,a_nested_as.dot_symbol,Void,Void)
			end
		ensure
			not_void: Result /= Void
		end
	...
	bpl_type_for (type: TYPE_AS): STRING is
		...
		do
			...
			like_id_t ?= type
			if cls_t /= Void then
				if mapping_table.item(cls_t.class_name) /= Void then
					Result := mapping_table.item (cls_t.class_name)
