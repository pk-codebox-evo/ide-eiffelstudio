note
	description: "Command line options for semantic search facilities"
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature {NONE} -- Initialization
	...
	make_with_arguments (a_arguments: LINKED_LIST [STRING])
		...
		do
			create {LINKED_LIST [STRING]} semantic_search_arguments.make
			a_arguments.do_all (agent semantic_search_arguments.extend)
		ensure
			arguments_set: semantic_search_arguments /= Void and then semantic_search_arguments.count = a_arguments.count
		end

feature -- Access

	semantic_search_arguments: LINKED_LIST [STRING];
			-- Arguments to semantic search

feature -- Properties

	name: STRING
		do
			Result := "Sementic search"
		end
	...
	execute
		...
		do
			...
			elseif l_config.should_generate_arff then
				create l_generate_arff_cmd.make (l_config)
				l_generate_arff_cmd.execute
