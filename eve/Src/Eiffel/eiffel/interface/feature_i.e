...
feature -- Setting
	...
	set_export_status (e: EXPORT_I)
		...
		do
			if
				(written_in = system.any_id and then feature_name_id = {PREDEFINED_NAMES}.default_create_name_id)
			...
			then
				-- The code generated by the SCOOP degree requires the feature 'default_create' of class 'ANY' to be exported to the proxy class of 'ANY'.
				-- Other features:
				-- ROUTINE feature_id required by the SCOOP profiler
				-- ROUTINE class_id required by the SCOOP profiler
				-- After degree 4 all compiled features are stored on the disk and cashed in memory. To change the export status permanently we need to make sure our change gets written to the disk. We can achieve this by overwriting the export status upon creation of the compiled feature.
				internal_export_status := Void
				feature_flags := feature_flags.set_bit_with_mask (False, is_export_status_none_mask)
			else
...
debug ("ACTIVITY")
	...
	io.error.put_new_line
					...
					elseif False and then not ( -- new SCOOP redeclaration rules page 187
						          new_type.conform_to (current_class, old_type) and then
						          ...
						          -- (new_type.processor_tag.is_equal (old_type.processor_tag) or else new_type.processor_tag.top)
						          True
						         ) then
						create vdrd53
						vdrd53.init (old_feature, Current)
