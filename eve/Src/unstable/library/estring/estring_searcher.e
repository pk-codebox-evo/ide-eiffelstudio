			...
			if a_string = a_pattern then
				...
				loop
					...
					until
						j = l_pattern_count
					loop
						if a_string.item (i + j - 1).natural_32_code /= a_pattern.code (j + 1) then
								-- Mismatch, so we stop
					...
					if l_matched then
						...
						if i + l_pattern_count <= end_pos then
							l_char_code := a_string.item (i + l_pattern_count - 1).code
							...
							if l_matched then
								...
								if i + l_pattern_count <= end_pos then
									...
									until
										j > fuzzy
									loop
										l_char_code := a_string.item (i + l_pattern_count - j - 1).code
										if l_char_code > max_code_point_value then
