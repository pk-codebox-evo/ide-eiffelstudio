note
	description: "Structural copy from {AST_VISITOR} to add pre-process methods for AST iteration."
	date: "$Date$"
	revision: "$Revision$"

deferred class
	EPA_AST_VISITOR_PRE_PROCESS_HOOK
...
feature -- Quantification
	...
	pre_process_there_exists_as (a_as: THERE_EXISTS_AS)
			-- Process `a_as'.
		deferred
		end

	pre_process_for_all_as (a_as: FOR_ALL_AS)
			-- Process `a_as'.
		deferred
		end
