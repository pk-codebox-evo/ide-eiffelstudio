...
feature -- Status report
	...
	is_local_detected: BOOLEAN
			-- Is there any local variable detected during last `qualify'?

	is_old_detected: BOOLEAN
			-- Is there any old expression detected during last `qualify'?

feature -- Access

	last_expression: STRING
			-- Last expression generated by last `qualify'

feature -- Basic operation

	qualify (a_expression: EPA_EXPRESSION; a_replacements: HASH_TABLE [STRING, STRING])
	...
	qualify_only_current (a_expression: EPA_EXPRESSION)
			-- Qualify `a_expression' only with "Current".
		local
			l_replacements: HASH_TABLE [STRING, STRING]
		do
			create l_replacements.make (1)
			l_replacements.compare_objects
			l_replacements.force (ti_current + ".", "")
			qualify (a_expression, l_replacements)
		end
