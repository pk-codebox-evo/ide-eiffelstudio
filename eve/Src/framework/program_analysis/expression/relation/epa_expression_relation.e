		...
		do
			...
			if attached {ETR_CLASS_CONTEXT} a_context as l_class_ctxt then
				...
				across invariant_of_class (l_class_ctxt.context_class) as l_invariants loop
					l_invariants.item.ast.process (l_relevancy_finder)
				end
			elseif attached {ETR_FEATURE_CONTEXT} a_context as l_feat_ctxt then
				...
				l_feat_ctxt.written_feature.e_feature.ast.process (l_relevancy_finder)
				across precondition_of_feature (l_feat_ctxt.written_feature, l_feat_ctxt.context_class) as l_preconditions loop
					l_preconditions.item.ast.process (l_relevancy_finder)
				end
				across postcondition_of_feature (l_feat_ctxt.written_feature, l_feat_ctxt.context_class) as l_postconditions loop
					l_postconditions.item.ast.process (l_relevancy_finder)
				end
			end
			if
				a_should_merge
			then
				l_relevancy_finder.merge_not_disjoint_sets
			end
			Result := l_relevancy_finder.set_of_relevant_expressions (a_expr)
		ensure
...
feature -- Status report
	...
	is_expression_relevant (a_expr: EPA_EXPRESSION; b_expr: EPA_EXPRESSION; a_context: ETR_CONTEXT; a_should_merge: BOOLEAN): BOOLEAN
		...
		do
			...
			Result := l_relevancy_finder.is_relevant (a_expr, b_expr)
		end
...
feature -- Related Expressions

	related_visit_feature (a_class_name: STRING; a_feature_name: STRING)
			--
		require
		...
		local
			l_class: CLASS_C
			l_feature: FEATURE_I
			l_printer: EPA_RELEVANT_EXPRESSION_FINDER
...
--			create l_printer.make (
			...
			l_printer.merge_not_disjoint_sets
			l_printer.dump_file ("/home/marc/Desktop/")
		end

	related_visit_class (a_class_name: STRING)
			--
		require
			a_class_name_not_void: a_class_name /= Void
		local
