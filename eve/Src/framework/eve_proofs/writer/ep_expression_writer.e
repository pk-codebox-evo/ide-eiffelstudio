...
feature -- Element change
	...
	set_context_target_type (a_type: like last_target_type)
			-- Set `last_target_type' to `a_type'.
		do
			context_target_type := a_type
		ensure
			context_target_type_set: context_target_type = a_type
		end
...
--			expression.put (name_mapper.heap_name + "[" + name_mapper.target_name + ", " + l_field_name + "]")
	...
	process_current_b (a_node: CURRENT_B)
			-- Process `a_node'.
		do
			last_target_type := a_node.type
			expression.put (name_mapper.current_name)

--			check last_target_type = context_target_type end
		end
	...
	process_routine_creation_b (a_node: ROUTINE_CREATION_B)
		...
		do
			...
			l_agent_class := system.class_of_id (a_node.class_id)
...
feature {NONE} -- Implementation
	...
	context_target_type: TYPE_A
			-- Type of surrounding context
	...
	process_normal_feature_call (a_feature: !FEATURE_I; a_parameters: BYTE_LIST [PARAMETER_B])
		...
		do
			...
			if a_feature.is_attribute then
				...
				check context_target_type /= Void end
				l_last_target_type := last_target_type
				last_target_type := context_target_type
					-- Store expression
				l_temp_expression := expression.string
					-- Evaluate parameters with fresh expression
				expression.reset
				...
				last_target_type := l_last_target_type
