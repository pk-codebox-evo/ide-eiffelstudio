	...
	think
		...
		do
			...
			then
				...
				if current_class.is_compiled and then
					(current_class.is_static_score_stale or
					 current_class.static_score = {EBB_VERIFICATION_SCORE}.not_verified or
					 current_class.is_dynamic_score_stale or
					 current_class.dynamic_score = {EBB_VERIFICATION_SCORE}.not_verified)
				then
					create l_input.make
					l_input.add_class (current_class.compiled_class)
					if current_class.is_static_score_stale or current_class.static_score = {EBB_VERIFICATION_SCORE}.not_verified then
						l_tool := blackboard.default_tool_of_type ({EBB_TOOL_CATEGORY}.static_verification)
					elseif current_class.is_dynamic_score_stale or current_class.dynamic_score = {EBB_VERIFICATION_SCORE}.not_verified then
						l_tool := blackboard.default_tool_of_type ({EBB_TOOL_CATEGORY}.dynamic_verification)
					else
						check False end
					end
