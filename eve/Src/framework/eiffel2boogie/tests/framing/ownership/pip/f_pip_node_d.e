...
feature -- Access
	...
	ancestors: MML_SET [F_PIP_NODE_D]
	...
	descendants: MML_SET [F_PIP_NODE_D]
	...
	acquire (n: F_PIP_NODE_D)
		...
		do
			...
			if not (n.descendants <= descendants) then
				update_descendants (n, n, {MML_SET [F_PIP_NODE_D]}.empty_set)
			end
			if n.value > value then
				update_value (n, n, {MML_SET [F_PIP_NODE_D]}.empty_set)
			else
				wrap
			end
		ensure
...
feature {F_PIP_NODE_D} -- Implementation
	...
	update_value (child, d: F_PIP_NODE_D; visited: MML_SET [F_PIP_NODE_D])
	...
	update_ancestors (a: F_PIP_NODE_D)
	...
	update_descendants (child, d: F_PIP_NODE_D; visited: MML_SET [F_PIP_NODE_D])
...
invariant
	...
	ancestors_consistent: ancestors = (if parent = Void then {MML_SET [F_PIP_NODE_D]}.empty_set else parent.ancestors & parent end) & Current
