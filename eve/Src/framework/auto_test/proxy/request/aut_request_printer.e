...
feature {AUT_REQUEST} -- Processing
	...
	process_create_object_request (a_request: AUT_CREATE_OBJECT_REQUEST)
		...
		do
			...
			print_execute_request (l_compound, execute_request_flag, extra_data_for_execute_request (a_request), a_request.class_of_target_type.name_in_upper + "." + a_request.creation_procedure.feature_name)
		end
...
--			l_receiver_type := l_feature.type.actual_type.instantiation_in (l_target_type, l_feature.written_in)
			...
			print_execute_request (l_compound, execute_request_flag, extra_data_for_execute_request (a_request), a_request.class_of_target_type.name_in_upper + "." + a_request.feature_name)
		end
...
feature {NONE} -- Byte code generation
	...
	print_execute_request (a_compound: BYTE_LIST [BYTE_NODE]; a_request_flag: NATURAL_8; a_extra_data: detachable ANY; a_feature_name: detachable STRING)
			...
			-- `a_feature_name' is the name of the feature under test (if any, it can be none because a request can be doing something else than
			-- testing a feature.
		require
		...
		do
			...
			last_request := [a_request_flag, [l_byte_code_data, a_feature_name, l_extra]]
		end
