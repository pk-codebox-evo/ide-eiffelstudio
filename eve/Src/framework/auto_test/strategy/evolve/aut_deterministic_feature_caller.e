...
class AUT_DETERMINISTIC_FEATURE_CALLER
...
feature {NONE} -- Implementation
	...
	queue: AUT_DYNAMIC_PRIORITY_DETERMINISTIC_QUEUE
			-- Queue
...
feature {NONE} -- Steps
			...
			if target /= Void  then
				...
				if not type.is_none and then not type.is_expanded then
					...
					if feature_to_call /= Void then
						...
						add_feature_argument_type_in_deterministic_input_creator (feature_to_call, type, target_creator)
	...
	create_target_creator
		...
		do
			if interpreter.configuration.is_object_state_exploration_enabled then
				target_creator := create {AUT_DETERMINISTIC_UNTRIED_SOURCE_OBJECT_STATE_INPUT_CREATOR}.make (system, interpreter, feature_table, feature_to_call, type)
			else
				create target_creator.make (system,interpreter, feature_table)
	...
	create_argument_creator
		...
		do
			...
			add_feature_argument_type_in_deterministic_input_creator (feature_to_call, type, argument_creator)
