...
class AUT_DETERMINISTIC_OBJECT_CREATOR
...
feature -- Access
	...
	input_creator: AUT_DETERMINISTIC_INPUT_CREATOR
...
feature {NONE} -- Steps
	...
	create_input_creator
		...
		do
			...
			if creation_procedure.arguments /= Void then
				add_feature_argument_type_in_deterministic_input_creator (creation_procedure, type, input_creator)
			end
			input_creator.start
		end
	...
	choose_expanded_receiver
		...
		do
			class_ := type.associated_class
			if parameter_loader.is_evolving_primitive then --Select from evolved values
...
choose_boolean_constant is
	...
	queue: detachable AUT_DYNAMIC_PRIORITY_DETERMINISTIC_QUEUE
			-- Queue
