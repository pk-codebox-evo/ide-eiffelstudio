note
	description: "Summary description for {AUT_FAILED_PRECONDITION_PROPOSAL_OBSERVER}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"

class
	AUT_FAILED_PRECONDITION_PROPOSAL_OBSERVER
...
feature -- Access
	...
	data_anchor_type: TUPLE [class_name: STRING; feature_name: STRING; predicate: STRING; time: INTEGER]
	...
	process_comment_line (a_line: STRING) is
		...
		do
			...
			if a_line.substring (1, l_count).is_equal (l_header) then
				...
				l_predicate_name.left_adjust
				l_predicate_name.right_adjust

				l_parts := l_parts.i_th (3).split (':')
				l_parts.i_th (2).left_adjust
				l_parts.i_th (2).right_adjust
				l_time := l_parts.i_th (2).to_integer
				l_data := [l_class_name, l_feature_name, l_predicate_name, l_time]
...
feature{NONE} -- Implementation
	...
	precondition_eval_header: STRING is "-- Failed predicate: "
