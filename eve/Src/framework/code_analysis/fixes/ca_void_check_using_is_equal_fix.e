...
feature {NONE} -- Visitor
	...
	process_nested_as (a_nested_as: NESTED_AS)
		local
			l_new_text: STRING_32
		do
			if a_nested_as.is_equivalent (nested_to_change) then
				create l_new_text.make_empty

				l_new_text.append ("(")
				l_new_text.append (a_nested_as.target.access_name_32)
				l_new_text.append (" = Void")
				l_new_text.append (")")

				a_nested_as.replace_text (l_new_text, matchlist)
			end
		end
