note
	description: "[
					RULE #63: Class naming convention violated
		
					Class names should respect the Eiffel naming convention for classes
					(all uppercase, no trailing or two consecutive underscores).
	]"
	author: "Paolo Antonucci"
	date: "$Date$"
	revision: "$Revision$"

class
	CA_CLASS_NAMING_CONVENTION_RULE

inherit

	CA_STANDARD_RULE

create
	make

feature {NONE} -- Initialization

	make
			-- Initialization for `Current'.
		do
			make_with_defaults
		end

feature {NONE} -- Activation

	register_actions (a_checker: attached CA_ALL_RULES_CHECKER)
		do
			a_checker.add_class_pre_action (agent process_class)
		end

feature {NONE} -- Rule checking

	process_class (a_class_as: attached CLASS_AS)
		local
			l_name: STRING
			l_viol: CA_RULE_VIOLATION
		do
			l_name := a_class_as.class_name.name_8
			if not is_valid_class_name (l_name) then
				create l_viol.make_with_rule (Current)
				l_viol.set_location (a_class_as.class_name.start_location)
				violations.extend (l_viol)
			end
		end

	is_valid_class_name (a_name: attached STRING): BOOLEAN
			-- Currently the casing restriction cannot be enforced, as identifiers received by this
			-- function are always upper- or lower-cased.
		do
			Result := not a_name.ends_with ("_") and not a_name.has_substring ("__") and (a_name.as_upper ~ a_name)
		end

feature -- Properties

	title: STRING_32
		do
			Result := ca_names.class_naming_convention_title
		end

	id: STRING_32 = "CA063"
			-- <Precursor>

	description: STRING_32
		do
			Result := ca_names.class_naming_convention_description
		end

	format_violation_description (a_violation: attached CA_RULE_VIOLATION; a_formatter: attached TEXT_FORMATTER)
		do
			a_formatter.add (ca_messages.class_naming_convention_violation_1)
			check attached a_violation.affected_class as affected_class then
				a_formatter.add_class (affected_class.original_class)
			end
			a_formatter.add (ca_messages.class_naming_convention_violation_2)
		end

end
