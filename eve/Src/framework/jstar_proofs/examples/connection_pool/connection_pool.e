note
	...
	revision: "$Revision$"
	sl_predicate: "[
	DBPool(x, {type=t; last=l}) = t = sql(_dm,_url,_user,_password) * x.<CONNECTION_POOL.driver_manager> |-> _dm * x.<CONNECTION_POOL.url> |-> _url * x.<CONNECTION_POOL.user> |-> _user * x.<CONNECTION_POOL.password> |-> _password * x.<CONNECTION_POOL.conns> |-> _y * list(_y, Void, _R) * DBSet(setof(_R), t) * x.<CONNECTION_POOL.last_connection> |-> l
	]"
