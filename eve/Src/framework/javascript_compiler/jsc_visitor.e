note
	description : "A schematic implementation for a byte node visitor."
	author      : "Alexandru Dima <alex.dima@gmail.com>"
	copyright   : "Copyright (C) 2011, Alexandru Dima"
	date        : "$Date$"
	revision    : "$Revision$"

deferred class JSC_VISITOR
...
feature {BYTE_NODE} -- Visitors
	...
	process_loop_expr_b (a_node: LOOP_EXPR_B)
			-- Process `a_node'.
		local
			l_iteration_code: BYTE_LIST [BYTE_NODE]
			l_iteration_exit_condition_code: EXPR_B
			l_expression_code: EXPR_B
			l_advance_code: BYTE_NODE
		do
			l_iteration_code := a_node.iteration_code
			check l_iteration_code /= Void end

			l_iteration_exit_condition_code := a_node.iteration_exit_condition_code
			check l_iteration_exit_condition_code /= Void end

			l_expression_code := a_node.expression_code
			check l_expression_code /= Void end

			l_advance_code := a_node.advance_code
			check l_advance_code /= Void end

			l_iteration_code.process (Current)
			safe_process (a_node.invariant_code)
			safe_process (a_node.variant_code)
			l_iteration_exit_condition_code.process (Current)
			safe_process (a_node.exit_condition_code)
			l_expression_code.process (Current)
			l_advance_code.process (Current)
		end
