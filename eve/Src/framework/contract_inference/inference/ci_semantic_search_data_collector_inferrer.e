	...
	process_transitions
		...
		do
			across data.transition_data as l_transitions loop
				if attached {SEM_FEATURE_CALL_TRANSITION} l_transitions.item.transition as l_transition then
					create l_transition_writer
					create l_file.make_create_read_write (tranisition_file_path (l_transition))
...
feature{NONE} -- Implementation
	...
	test_case_info_fields (a_transition: SEM_FEATURE_CALL_TRANSITION; a_tc_info: CI_TEST_CASE_INFO): DS_HASH_SET [SEM_DOCUMENT_FIELD]
		...
		do
			...
			Result.force_last (create {SEM_DOCUMENT_FIELD}.make (once "test_case_class", a_tc_info.test_case_class.name_in_upper, string_field_type, default_boost_value))
			Result.force_last (create {SEM_DOCUMENT_FIELD}.make (once "test_feature", a_tc_info.test_feature.feature_name.as_lower, string_field_type, default_boost_value))
			Result.force_last (create {SEM_DOCUMENT_FIELD}.make (once "is_feature_under_test_query", a_tc_info.is_feature_under_test_query.out, boolean_field_type, default_boost_value))
			Result.force_last (create {SEM_DOCUMENT_FIELD}.make (once "is_feature_under_test_creation", a_tc_info.is_feature_under_test_creation.out, boolean_field_type, default_boost_value))
			Result.force_last (create {SEM_DOCUMENT_FIELD}.make (once "operand_variable_indexes", a_tc_info.operand_variable_indexes, string_field_type, default_boost_value))
		end
	...
	tranisition_file_path (a_transition: SEM_FEATURE_CALL_TRANSITION): STRING
		...
		do
			...
			l_path.set_file_name (a_transition.class_.name_in_upper + "__" + a_transition.feature_.feature_name.as_lower + "__" + a_transition.uuid + ".tran")
			Result := l_path.out
		end
