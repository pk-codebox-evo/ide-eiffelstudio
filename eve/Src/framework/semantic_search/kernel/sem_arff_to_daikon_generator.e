...
feature -- Access
	...
	feature_: detachable FEATURE_I
			-- Feature on which invariants are to be inferred
			-- This is optional, when it is set, the comparability
			-- of integer expressions will be more accurate.

feature -- Status report

	is_missing_value_included: BOOLEAN
			-- Should expressions with missing values be included?
			-- Default: False

feature -- Settings

	set_is_missing_value_included (b: BOOLEAN)
			-- Set `is_missing_value_included' with `b'.
		do
			is_missing_value_included := b
		ensure
			is_missing_value_included_set: is_missing_value_included = b
		end
...
feature -- Basic operations
	...
	generate (a_relation: WEKA_ARFF_RELATION)
		...
		do
			arff_relation := a_relation.cloned_object
			if not is_missing_value_included then
				trim_arff_relation
			end

				-- Calculate relevant expressions for operands of `feature_', if set.
			create integer_expression_comparability.make (50)
			integer_expression_comparability.set_key_equality_tester (expression_equality_tester)
			if context_class /= Void and then feature_ /= Void then
				calculate_relevant_expressions
			end
...
feature{NONE} -- Implementation
	...
	relevant_expressions: DS_HASH_TABLE [DS_HASH_SET [EPA_EXPRESSION], EPA_EXPRESSION]
			-- Expressions that are relevant to operands of `feature_', if it is attached
			-- Keys are operands of `feature_', values are expressions that relevant to
			-- those operands.
	...
	variable_declaraction (a_attribute: WEKA_ARFF_ATTRIBUTE): DKN_VARIABLE
		...
		do
			...
			if l_prefixes.i_th (2) ~ once "i" then
				...
				if context_class /= Void and then feature_ /= Void then
					l_expr_text := string_slices (a_attribute.name, "::").last
					l_expr := expression_from_curly_braced_operands (context_class, feature_, l_expr_text)
					if l_expr.type /= Void then
						integer_expression_comparability.search (l_expr)
						if integer_expression_comparability.found then
							l_comparability := integer_expression_comparability.found_item
							l_int_done := True
						end
					end
				end

				if not l_int_done then
...
--				l_comparability := boolean_comparability
				l_comparability := next_boolean_comparability
				next_boolean_comparability := next_boolean_comparability + 1
			elseif l_prefixes.i_th (2) ~ once "r" then
	...
	calculate_relevant_expressions
			-- Calculate expressions that are relevant to operands of `feature_' and
			-- populate `relevant_expressions'.
		local
			l_finder: EPA_INTERESTING_EXPRESSION_FINDER
		...
		do
			...
			l_operand_types := resolved_operand_types_with_feature (feature_, context_class, context_class.constraint_actual_type)
			create l_finder
			l_finder.find (context_class, feature_)
			relevant_expressions := l_finder.last_relevant_expressions

			from
				l_cursor := relevant_expressions.new_cursor
				l_cursor.start
			until
				l_cursor.after
			loop
				l_opd_name := l_cursor.key.text
				l_opd_index := l_operands.item (l_opd_name)
				if l_operand_types.item (l_opd_index).is_integer then
					integer_expression_comparability.force_last (l_opd_index, l_cursor.key)
					from
					...
					until
						l_cursor2.after
					loop
						integer_expression_comparability.force_last (l_opd_index, l_cursor2.item)
						l_cursor2.forth
					end
				end
				l_cursor.forth
	...
	integer_expression_comparability: DS_HASH_TABLE [INTEGER, EPA_EXPRESSION]
			-- Comparability among integer expressions
			-- Keys are expressions, values are their comparability level
