...
inherit
	ARRAYED_LIST [ES_EVE_AUTOFIX_FIXING_SUGGESTION]
...
feature{NONE} -- Initialization
	...
	make (a_fault_signature: STRING)
			-- Initialize an AutoFix result for `a_fault_signature', from the AutoFix results directory of the current project.
		require
		...
		do
			...
			create l_file_name.make_from_string (eiffel_project.project_directory.fixing_results_path)
			l_file_name.set_file_name (a_fault_signature)
			l_file_name.add_extension ("afr")
			result_file_name := l_file_name
...
feature -- Basic operations
	...
	refresh
			-- Refresh the result from `result_file_name'.
		local
			l_current_time_stamp: INTEGER
		do
			is_completed := is_associated_fixing_finished
			if is_completed then
				l_current_time_stamp := file_system.file_time_stamp (result_file_name)
				if l_current_time_stamp > time_stamp then
						-- Result file updated since last load. Refresh it.
					load
				end
			else
				reset
	...
	apply_fix (a_suggestion: ES_EVE_AUTOFIX_FIXING_SUGGESTION)
		...
		require
			...
			suggestion_from_result: Current[a_suggestion.index] = a_suggestion
		local
		...
		do
			...
			if l_should_backup then
				...
				if l_dir.exists and l_dir.is_writable then
					...
					create l_file.make (l_backup_file_name)
					if not l_file.exists then
						create l_exception.make ("[Backup] Creation of " + l_backup_file_name + " failed.")
						l_exception.raise
					end
				else
					create l_exception.make ("[Backup] Directory does not exist or is not writable: " + l_dir.name)
					l_exception.raise
				end
			end

				-- Update the faulty feature body.
			...
			if l_file.is_open_write then
				...
				create l_exception.make ("[Update] Writing to " + l_origin_file_name + " failed.")
				l_exception.raise
			end
			...
			if l_file.is_open_append then
				l_file.put_string ("%N" + Tag_valid_fix_applied + ":" + a_suggestion.index.out + "%N")
				if l_should_backup then
					l_file.put_string (Tag_backup_class_file + ":" + l_backup_file_name + "%N")
				end
				l_file.close
			end
	...
	revert
		...
		local
			...
			l_backup_file, l_system_file, l_result_file: PLAIN_TEXT_FILE
		...
		do
			...
			file_system.copy_file (backup_class_file, l_system_file_name)
				-- Compare the size after copying.
			create l_system_file.make (l_system_file_name)
			l_size_system := l_system_file.count
			check l_size_backup = l_size_system end

				-- Update result file.
			create l_result_file.make (result_file_name)
			l_result_file.open_append
			if l_result_file.is_open_append then
				l_result_file.put_string (Tag_last_fix_reverted + "%N")
				l_result_file.close
			end
...
feature -- Status report
	...
	is_completed: BOOLEAN
			-- Is result file completed by the associated fixing session?		

	is_well_formed: BOOLEAN
			-- Is `Current' a well-formed AutoFix result?
...
feature -- Access
	...
	short_summary_text: STRING
		...
		do
			if is_fixed then
				Result := fault_signature + " (Fixed)"
			else
				Result := fault_signature.twin
			end
		end
...
feature {NONE} -- Implementation
				...
				if l_file.is_open_read then
					...
					loop
						...
						elseif l_line.ends_with (Tag_valid_fix_end) then
							l_inside_valid_fix := False
							create l_suggestion.make (l_index, Current, faulty_class, faulty_feature, faulty_feature_text, l_fix_content)
							force (l_suggestion)
							create l_fix_content.make (1024)
							l_index := l_index + 1
					...
					l_file.close
					time_stamp := file_system.file_time_stamp (result_file_name)
				end
			end
		rescue
			l_retried := True
	...
	is_associated_fixing_finished: BOOLEAN
...
feature -- Constants
	...
	Tag_valid_fix_applied: 	 STRING = "Valid_fix_applied"
	Tag_backup_class_file:	 STRING = "Backup_up_class_file"
	Tag_last_fix_reverted:   STRING = "Last_fix_reverted"
