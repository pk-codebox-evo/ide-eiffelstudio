...
feature -- Access
	...
	contract_expressions_for_features: DS_HASH_TABLE [TUPLE [pre, post: EPA_HASH_SET [EPA_EXPRESSION]], AFX_FEATURE_TO_MONITOR]
		do
	...
	feature_contracts: DS_HASH_TABLE [TUPLE[pre, post: EPA_HASH_SET[EPA_EXPRESSION]], AFX_FEATURE_TO_MONITOR]
...
feature{NONE} -- Implementation
	...
	register_program_state_monitoring
		...
		local
			...
			l_bpt_precondition, l_bpt_postcondition: INTEGER
		do
...
--				l_contract_expressions.post.do_all (agent l_expressions.force)
				...
				l_manager.set_breakpoint_with_expression_and_action (l_feature_to_monitor.breakpoint_to_evaluate_precondition,
																	 l_contract_expressions.pre, agent on_breakpoint_hit_in_test_case (l_class, l_feature, ?, ?))
				l_manager.set_breakpoint_with_expression_and_action (l_feature_to_monitor.breakpoint_to_evaluate_postcondition,
...
--	expressions_for_contracts (a_feature: AFX_FEATURE_TO_MONITOR): TUPLE [pre, post: EPA_HASH_SET [EPA_EXPRESSION]]
...
--			l_expression, l_failing_precondition_in_recipient: EPA_EXPRESSION
--			l_expressions, l_precondition_expressions, l_postcondition_expressions, l_post_contracts, l_pre_contracts, l_all_contracts: EPA_HASH_SET[EPA_EXPRESSION]
...
--			l_sub_expressions_from_contracts: TUPLE[pre, post: DS_HASH_SET [EPA_EXPRESSION]]
--			l_is_creation_procedure: BOOLEAN
--			l_extra_precondition_expressions: DS_ARRAYED_LIST[EPA_EXPRESSION]
--		do
...
--				if session.exception_from_execution.is_precondition_violation and then a_feature ~ features_on_stack.item (2) and then attached session.exception_from_execution.exception_condition_in_recipient as lt_failing_condition then
...
--	sub_expressions_from_contracts (a_feature: AFX_FEATURE_TO_MONITOR): TUPLE [pre, post: DS_HASH_SET [EPA_EXPRESSION]]
...
--		local
--			l_exps, l_all_exprs, l_pre_exprs, l_post_exprs: EPA_HASH_SET [EPA_EXPRESSION]
--			l_pre_contracts, l_post_contracts: EPA_HASH_SET [EPA_EXPRESSION]
...
--					l_cursor.after
--				loop
--					l_sub_expression_collector.collect_from_expression (a_feature, l_cursor.item)
...
--	extra_precondition_expressions (a_feature: AFX_FEATURE_TO_MONITOR): DS_ARRAYED_LIST [EPA_EXPRESSION]
--		local
...
--		end
	...
	variant_postconditions: EPA_HASH_SET [EPA_EXPRESSION]
		local
			l_failing_condition, l_expression, l_new_condition: EPA_EXPRESSION
			l_failing_condition_text, l_text, l_new_condition_text: STRING
			l_sub_expression_collector: EPA_SUB_EXPRESSION_COLLECTOR
			l_sub_expressions: EPA_HASH_SET [EPA_EXPRESSION]
			l_expression_cursor: DS_HASH_SET_CURSOR [EPA_EXPRESSION]
		...
		do
			...
			if session.exception_from_execution.is_postcondition_violation then
				l_failing_condition := session.exception_from_execution.exception_condition_in_recipient
				l_failing_condition_text := l_failing_condition.text
				create l_sub_expression_collector
				l_sub_expression_collector.collect_from_expression (session.exception_from_execution.recipient_feature_with_context, l_failing_condition)
...
--	expressions_capturing_argument_relations (a_feature: AFX_FEATURE_TO_MONITOR): EPA_HASH_SET [EPA_EXPRESSION]
--			--
--		local
...
--	derived_expressions (a_feature: EPA_FEATURE_WITH_CONTEXT_CLASS; a_expressions_to_extend, a_expressions_to_use: EPA_HASH_SET [EPA_EXPRESSION]): EPA_HASH_SET[EPA_EXPRESSION]
--			-- Set of derived expressions from `a_expression_texts', in the context of `a_feature'.
--		local
--			l_component_expressions: EPA_HASH_SET [EPA_EXPRESSION]
--			l_boolean_expressions, l_integer_expressions: EPA_HASH_SET [EPA_EXPRESSION]
