note
	description: "Summary description for {AFX_INTEGRAL_EXPRESSION_COMBINATION_WITHIN_BREAKPOINT}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"

class
	AFX_INTEGRAL_EXPRESSION_COMBINATION_WITHIN_BREAKPOINT
...
feature	-- Basic operation
	...
	combinations_with_indexes (a_class: CLASS_C; a_feature: FEATURE_I; a_integrals: DS_HASH_TABLE [DS_HASH_SET [INTEGER], STRING]; a_combinations: LINKED_LIST [EPA_HASH_SET [STRING]])
			...
			-- <Precursor>
			-- Strategy that combines integral expressions from the same locations.
		local
		...
		do
			...
			loop
				...
				check text_registered: a_integrals.has (l_text1) end
				l_index_range1 := a_integrals.item (l_text1)
