...
class
	EPA_FEATURE_AST_STRUCTURE_NODE

inherit
	EPA_AST_STRUCTURE_NODE
		redefine
			is_feature_node
		end
...
feature -- Access
	...
	surrounding_instruction (a_bpslot: INTEGER): detachable EPA_AST_STRUCTURE_NODE
	...
	instructions_in_block (a_bpslot: INTEGER): LINKED_LIST [EPA_AST_STRUCTURE_NODE]
			-- Instructions that are in the same basic block including `a_bpslot'
			-- Empty list means no such instruction is found.			
		do
			create Result.make
			if attached {EPA_AST_STRUCTURE_NODE} surrounding_instruction (a_bpslot) as l_node then
				if attached {LINKED_LIST [EPA_AST_STRUCTURE_NODE]} l_node.sibling as l_sibling then
					l_sibling.do_if (
						agent Result.extend,
						agent (a_node: EPA_AST_STRUCTURE_NODE): BOOLEAN
							do
			...
			only_instruction_in_result:
				Result.for_all (agent (n: EPA_AST_STRUCTURE_NODE): BOOLEAN do Result := n.is_instruction end)
		end

	instructions_in_block_as (a_node: EPA_AST_STRUCTURE_NODE): LINKED_LIST [EPA_AST_STRUCTURE_NODE]
			-- Instructions that are in the same basic block as `a_node'. Result may include `a_node' itself.	
			-- Empty list means no such instruction is found.
		do
			create Result.make
			if attached {LINKED_LIST [EPA_AST_STRUCTURE_NODE]} a_node.sibling as l_sibling then
				l_sibling.do_if (
					agent Result.extend,
					agent (n: EPA_AST_STRUCTURE_NODE): BOOLEAN
						do
	...
	node_at_break_point (a_bpslot: INTEGER): detachable EPA_AST_STRUCTURE_NODE
	...
	node_at_break_point_internal (a_parent: EPA_AST_STRUCTURE_NODE; a_bpslot: INTEGER): detachable EPA_AST_STRUCTURE_NODE
		...
		local
			l_cursor: CURSOR
			l_trunks: LINKED_LIST [LINKED_LIST [EPA_AST_STRUCTURE_NODE]]
			l_children: LINKED_LIST [EPA_AST_STRUCTURE_NODE]
			l_cur2: CURSOR
		do
	...
	next_break_point (a_bpslot: INTEGER): INTEGER
		...
		do
			if attached {EPA_AST_STRUCTURE_NODE} node_at_break_point_internal (Current, a_bpslot) as l_node then
				Result := next_break_point_internal (l_node)
			end
		end

	last_node_with_break_point (a_node: EPA_AST_STRUCTURE_NODE): detachable EPA_AST_STRUCTURE_NODE
	...
	first_node_with_break_point (a_node: EPA_AST_STRUCTURE_NODE): detachable EPA_AST_STRUCTURE_NODE
...
feature{NONE} -- Implementation
	...
	next_break_point_internal (a_node: EPA_AST_STRUCTURE_NODE): INTEGER
				...
				if attached {LINKED_LIST [EPA_AST_STRUCTURE_NODE]} l_node.sibling as l_sibling then
					...
					loop
						if l_self_found then
							if attached {EPA_AST_STRUCTURE_NODE} first_node_with_break_point (l_sibling.item_for_iteration) as l_node_with_bp then
