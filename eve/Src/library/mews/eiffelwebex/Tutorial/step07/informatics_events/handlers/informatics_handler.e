...
feature {INFORMATICS_HANDLER} -- processing request
	...
	post_processing is
		...
		do
			check
				script_name_valid: context.config.form_action /= void and not context.config.form_action.is_empty
			end
			...
			return_view.replace_marker_with_string ("CGI_FILE_NAME", context.config.form_action)
		...
		do
			if actual_user.role = role_normal_user then
				create s_menu.make_from_string("{
	        					<span><a href="{#CGI_FILE_NAME#}?event&amp;cmd=list">Home</a></span>
	              				<span><a href="{#CGI_FILE_NAME#}?event&amp;cmd=eventform&amp;mode=add">Submit Event</a></span>
	              				<span><a href="{#CGI_FILE_NAME#}?event&amp;cmd=ownevents">Submitted Events</a></span>
				}")
				create s_username.make_from_string("{
								<a href="{#CGI_FILE_NAME#}?user&amp;cmd=details">{#USER_NAME#}</a>&nbsp;|&nbsp;
								<a href="{#CGI_FILE_NAME#}?user&amp;cmd=userform&amp;mode=update">Edit Profile</a>&nbsp;|&nbsp;
								<a href="{#CGI_FILE_NAME#}?event&amp;cmd=logout">Logout</a>
				}")
			...
			elseif actual_user.role = role_administrator then
				create s_menu.make_from_string("{
	        					<span><a href="{#CGI_FILE_NAME#}?event&amp;cmd=list">Home</a></span>
	              				<span><a href="{#CGI_FILE_NAME#}?event&amp;cmd=eventform&amp;mode=add">Submit Event</a></span>
	              				<span><a href="{#CGI_FILE_NAME#}?user&amp;cmd=list">View Users</a></span>
	              				<span><a href="{#CGI_FILE_NAME#}?user&amp;cmd=userform&amp;mode=add">Add User</a></span>
				}")
				create s_username.make_from_string("{
								<a href="{#CGI_FILE_NAME#}?user&amp;cmd=details">{#USER_NAME#}</a>&nbsp;|&nbsp;
								<a href="{#CGI_FILE_NAME#}?user&amp;cmd=userform&amp;mode=update">Edit Profile</a>&nbsp;|&nbsp;
				...
				create s_username.make_from_string("{
								Welcome, Guest&nbsp;|&nbsp;
								<a href="{#CGI_FILE_NAME#}?user&amp;cmd=userform&amp;mode=register">Register</a>&nbsp;|&nbsp;
								<a href="{#CGI_FILE_NAME#}?user&amp;cmd=loginform">Login</a>
				}")
			end
