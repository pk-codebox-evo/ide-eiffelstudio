...
feature {NONE} -- implementation
	...
	admin_user_list is
		...
		do
			...
			return_view.replace_marker_with_string ("USER_COUNT", user_manager.user_count.out)
			...
			if error_messages.count = 0 then
				if user_manager.is_user_authentication_valid(context.text_field_value ("user_id"), context.text_field_value ("password")) then
					...
					if a_user.status = user_suspended then
						...
						-- revalidate session expiration time
	...
	create_update_user_account is
		...
		do
			...
			if error_messages.count > 0 then
				...
				-- Regenerate a new captcha in case of errors
				captcha_gen.generate_question
				return_view.replace_marker_with_string ("question", captcha_gen.last_question)
				return_view.replace_marker_with_string ("questionid", captcha_gen.last_question_id)
...
--						error_string_table.put ("Given email address is already registered.", "Email")
			...
			if not validator.is_must_field_filled ("captcha") then
				error_string_table.put ("The security question must be answered", "No captcha")
			elseif
				not validator.is_captcha_valid (validator.get_field_string ("questionid"), validator.get_field_string ("captcha")) then
					error_string_table.put ("The answer to the security question is wrong", "Wrong captcha")
			end
