...
feature -- Queries with criteria
	...
	composite_search_criterion: PS_CRITERION
		...
		do
			...
			Result := factory [["first_name", "=", "Albo"]] and factory [["last_name", "=", "Bitossi"]] and not factory [[agent age_more_than(?, 20)]]
		end
	...
	update_ages
		...
		do
			...
			loop
				...
				executor.execute_update_within_transaction (query_result.item, transaction)
			end
			transaction.commit

				-- The commit may have failed.
			if transaction.has_error then
				if attached transaction.error.description as msg then
