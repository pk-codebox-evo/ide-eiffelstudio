note
	...
	file:        "$URL: https://svn.origo.ethz.ch/abel/trunk/apps/test/dadl_test/src/main/dadl_test_objects.e $"
...
feature -- Access
	...
	test_table: ARRAYED_LIST [TUPLE [test_input_tags:ARRAY[STRING]; test_proc:PROCEDURE [ANY, TUPLE[STRING]]; test_group_name:STRING]]
			-- List of tuples containing test data item names, test proc, test group name, suitable for populating an explorer
		once
			create Result.make(0)
			Result.extend ([round_trip_test_data.current_keys, agent round_trip, "Round trip tests"])
			Result.extend ([from_dadl_test_data.current_keys, agent from_dadl, "From dadl tests"])
		end

	from_dadl_test_data: HASH_TABLE [STRING, STRING]
			-- table of dadl test texts, read from files, keyed by their name
		local
			dir: DIRECTORY
	...
	round_trip_test_data: HASH_TABLE [ANY, STRING]
...
--			Result.set_my_string_32 ("And honey trees")
	...
	dadl_primitive_sequence_types_1: DADL_PRIMITIVE_SEQUENCE_TYPES
			-- sequences containing more than one value
		once
			create Result
	...
	dadl_primitive_sequence_types_2: DADL_PRIMITIVE_SEQUENCE_TYPES
			-- sequences containing single values only
		once
			create Result
...
feature -- Test procedures
	...
	round_trip (obj_key: STRING)
		local
			an_obj, new_obj: ANY
			dt: DT_COMPLEX_OBJECT_NODE
		do
			an_obj := round_trip_test_data.item (obj_key)
...
--			dadl_engine.set_full_type_marking_on
	...
	from_dadl (a_dadl_text_key: STRING)
		local
			a_dadl_text: STRING
		do
			a_dadl_text := from_dadl_test_data.item (a_dadl_text_key)
