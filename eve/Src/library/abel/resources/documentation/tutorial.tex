...
In this little tutorial, we are using \lstinline!PERSON! objects to show the usage of the API.
...
\begin{itemize}
 ...
 \item The \lstinline!OBJECT_QUERY [G]! class is used to describe a read operation in ABEL. 
...
To start using the library, we therefore need to create a \lstinline!REPOSITORY!.
...
The generic parameter \lstinline!G! of the \lstinline!OBJECT_QUERY! instance denotes the type of objects that should be queried.
...
The \lstinline!CRUD_EXECUTOR.is_persistent! feature can tell you if a specific object is known to ABEL and therefore has a link to its entry in the data\-base.
...
In ABEL, a basic type is an object of type \lstinline!STRING!, \lstinline!BOOLEAN!, \lstinline!CHARACTER! or any numeric object like \lstinline!REAL! or \lstinline!INTEGER!.
The \lstinline!PERSON! class has attributes that are of a basic type only, and those are stored together as a single unit in the database.
...
Instead of having a single object, ABEL has to insert a \lstinline!CHILD!'s mother and father as well, and if their parent attribute is attached as well it has to repeat again.
...
You need them to create the \lstinline!GENERIC_LAYOUT_SQL_BACKEND!, which you need in turn to create the \lstinline!RELATIONAL_REPOSITORY!.
...
That's all. You can now use the repository to create a \lstinline!CRUD_EXECUTOR! and start programming on it.
...
You can set a transaction isolation level using the feature \lstinline!REPOSITORY.set_transaction_isolation_level!.
...
The library will in general raise exceptions in case of an error and expose the error to the library user as an \lstinline!ERROR! object.
...
ABEL represents errors as objects, and all specific error classes inherit from the deferred class \lstinline!ERROR!.
...
\item \lstinline!UNRESOLVABLE_TRANSACTION_CONFLICT!: A conflict in implicit transaction management that doesn't resolve even after a retry
...
\item \lstinline!INTERNAL_ERROR!: Any error happening inside the library, like a failed SQL compilation.
...
You can get the actual error from the \lstinline!CRUD_EXECUTOR.error! or \lstinline!TRANSACTION.error! feature or - as \lstinline!ERROR! inherits from \lstinline!DEVELOPER_EXCEPTION! -
