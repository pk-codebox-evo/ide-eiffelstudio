...
We are assuming you have checked out the ABEL code from the EiffelStudio SVN repository\footnote{https://svn.eiffel.com/eiffelstudio/branches/eth/eve/Src/library/abel}, and have EiffelStudio installed. Launch it and in the initial window choose "persistence\_tutorial\_project". If it is not there just push "Add project" and navigate to the location where you downloaded ABEL, and look for the \emph{persistence\_tutorial\_project.ecf} project file in \emph{abel/apps/sample/tutorial/}. You can then load the project and you are ready to go. 

For convenience, in this tutorial we will use a simple in-memory repository simulating a relational database but storing all values in memory. To set up ABEL using a full-fledged relational back-end (like MySQL or SQLite) please read section ~\ref{section:advanced_initialization}.
...
\begin{lstlisting}[language=OOSC2Eiffel, captionpos=b, caption={A simple query.}, label={lst:simple_query}]
	simple_query: LINKED_LIST [PERSON]
		-- Query all person objects from the current active repository.
		local
...
Usually the result of such a query is very big, and you are probably only interested in objects that meet certain criteria, e.g. all persons of age 20. You can read more about it in section ~\ref{sec:query}.
...
Inserting and updating an object is done through features \lstinline{insert} and \lstinline{update} in \lstinline{PS_CRUD_EXECUTOR}: 

\begin{lstlisting}[language=OOSC2Eiffel, captionpos=b, caption={Insertion query.}, label={lst:simple_insert}]
...
Deletion is done through the feature \lstinline{delete} in \lstinline{PS_CRUD_EXECUTOR}, like in the following example:

\begin{lstlisting}[language=OOSC2Eiffel, captionpos=b, caption={Deletion query.}, label={lst:simple_delete}]
...
As you already know from Section~\ref{section:querying}, queries to a database are done by creating a  \lstinline!PS_OBJECT_QUERY[G]! object and using it from within a \lstinline!PS_CRUD_EXECUTOR!.
...
You can see how this is done in the technical documentation, where the object graph depth is described more detailed.
...
For MySQL, you need to create a \lstinline!PS_MYSQL_DATABASE! and \lstinline!PS_MYSQL_STRINGS! object.
You need them to create a \lstinline!PS_GENERIC_LAYOUT_SQL_BACKEND!, which you need in turn to create the \lstinline!PS_RELATIONAL_REPOSITORY!.
...
All examples from this tutorial work exactly the same, no matter if you use the \lstinline!PS_IN_MEMORY_REPOSITORY! or any of the database repositories.
...
This is done by manually creating an object of type \lstinline!TRANSACTION! and using the \lstinline!*_within_transaction! features in \lstinline!PS_CRUD_EXECUTOR! instead of the normal ones.
...
You can change the transaction isolation level by calling feature \lstinline{set_transaction_isolation_level} in  \lstinline{PS_REPOSITORY}.
...
You can get the actual error from the feature \lstinline!PS_CRUD_EXECUTOR.error! or \lstinline!TRANSACTION.error! or - due to the fact that the \lstinline!ERROR! class inherits from \lstinline!DEVELOPER_EXCEPTION! -
