...
For convenience, in this tutorial we will use a simple in-memory repository simulating a relational database but storing all values in memory. To set up ABEL using a full-fledged relational back-end (like MySQL or SQLite) please read Chapter ~\ref{chapter:advanced_initialization}.
...
We encourage you to test the features in class \lstinline!TUTORIAL! by inserting your code in feature \lstinline{make} (or by creating a new feature if you wish).
