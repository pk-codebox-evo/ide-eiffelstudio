...
For example, the two important data structures described above have to provide some kind of rollback mechanism, and ideally all ACID \todo{cite?} properties as well.
...
All object graph representation classes are in ``framework/object\_graph\_representation''. \todo{ a better way to include folders}
...
\begin{figure} [h!]
\centering
\includegraphics[trim= 0mm -10mm 0mm -10mm,clip, width = 13cm] {dataflow.png}
...
You need at least one handler for SPECIAL, because of its peculiarity that it doesn't have a fixed amount of fields, but you can include any other collection, e.g. a LIST or an ARRAY.
...
\todo{replace them with graphs from the tikz package}
\begin{figure} [!h]
\centering
...
Each operation will only handle the objects when the following condition holds: $ level(object) < depth $ \todo{don't inline this}
...
It provides a good interface to wrap a persistence mechanism that provides a similarly high level of abstraction, like for example db4o \todo{reference db4o}.
...
Due to its very flexible abstraction mechanism, you can easily extend ABEL with features like client-side transaction management or ESCHER \todo {reference escher} integration.
...
The layout is a modified version from the suggestion in Scott W. Ambler's article ``Mapping Objects to Relational Databases'' \todo{cite}.
...
The predefined backend located in ``backends/generic\_database\_layout''\todo{folder representation} maps Eiffel objects to this layout.
...
Please remember that the object-rlational mapping layer adds an attribute with name ``items\_owned''\todo{inline Eiffel?} to the ITEM object, which is the default behaviour for 1:N relations.
