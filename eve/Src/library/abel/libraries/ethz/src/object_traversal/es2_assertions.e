			...
			if expected /= actual then
				if expected /= Void and actual /= Void then
					...
					if visited.after then
						assert ("same types at " + path, expected.same_type (actual))
						visited.force ([expected, actual])
						assert_fields_equal (path, expected, actual, visited)
					else
						assert ("correct aliasing at " + path, visited.item.expected = expected and visited.item.actual = actual)
					end
				else
					assert ("same references at " + path, expected = Void and actual = Void)
				end
			end
		end
		...
		do
			...
			loop
				...
				when {INTERNAL}.boolean_type then
					assert_booleans_equal (assert_deep_equal_message (l_path),
						internal.boolean_field (i, expected),
						internal.boolean_field (i, actual))
				when {INTERNAL}.character_8_type then
					assert_characters_equal (assert_deep_equal_message (l_path),
						internal.character_field (i, expected),
						internal.character_field (i, actual))
				...
				when {INTERNAL}.integer_32_type then
					assert_integers_equal (assert_deep_equal_message (l_path),
						internal.integer_32_field (i, expected),
						internal.integer_32_field (i, actual))
