...
inherit --{NONE} TODO: restore non-conforming inheritance once the compiler bug has been fixed.
	...
	assert (a_tag: STRING; a_condition: BOOLEAN)
		do
			if not a_condition then
				...
				print ("%NReason: ")
				print (a_tag)
				assertions.assert ("deep_equal", False)
			end
		end
			...
			then
				...
				if is_marked (expected) then
					...
					until
						l_expected_found
					loop
						assertions.assert ("marked_implies_inserted", not l_equals.off)
