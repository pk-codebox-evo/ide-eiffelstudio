...
feature {NONE} -- Implementation
	...
	objects_deserialized: HASH_TABLE [ANY,INTEGER]
...
feature -- Creation features
	...
	make_with_config_path (path: STRING) is
...
feature --Serialization
	...
	serialize_with_path (obj: ANY; path: STRING) is
...
feature -- Deserialization
	...
	deserialize_with_default_path (file_name: STRING): ANY is
	...
	deserialize_object (obj: ANY): ANY is
	...
	deserialize_with_path (path: STRING): ANY is
		...
		do
			...
			create objects_deserialized.make (10)
				--create all tables
			id := 1
			ids.force (1)
...
feature -- Deserialization with schema evolution handler
	...
	-- deserialize_patch_with_default_path (file_name: STRING; table: DS_HASH_TABLE [SCHEMA_EVOLUTION_HANDLER, STRING]): ANY is
	deserialize_patch_with_default_path (file_name: STRING; table: HASH_TABLE [SCHEMA_EVOLUTION_HANDLER, STRING]): ANY is
	...
	-- deserialize_patch_object (obj: ANY;  table: DS_HASH_TABLE [SCHEMA_EVOLUTION_HANDLER, STRING]): ANY is
	deserialize_patch_object (obj: ANY;  table: HASH_TABLE [SCHEMA_EVOLUTION_HANDLER, STRING]): ANY is
	...
	-- deserialize_patch_with_path (path: STRING; table: DS_HASH_TABLE [SCHEMA_EVOLUTION_HANDLER, STRING]): ANY is
	deserialize_patch_with_path (path: STRING; table: HASH_TABLE [SCHEMA_EVOLUTION_HANDLER, STRING]): ANY is
			-- Deserialize an object at path.
		require
			path_exists: path/=Void and then not path.is_empty
			table_not_void: table/=Void
...
feature{NONE} --filter
	...
	is_filtered_class (obj: ANY): BOOLEAN is
	...
	filter (obj: ANY): CUSTOM_SERIALIZATION_FILTER is
			-- The filter
		local
			type: INTEGER
			tmp: FILTER_CLASS
...
feature{NONE} -- Recursive functions for serialization
	...
	create_string (obj: ANY; s: STRING): STRING is
...
feature{NONE} -- Recursive functions for deserialization
	...
	-- create_hash_table: DS_HASH_TABLE [ANY, STRING] is
	create_hash_table: HASH_TABLE [ANY, STRING] is
	...
	-- create_objects (table: DS_HASH_TABLE [ANY,STRING]): ANY is
	create_objects (table: HASH_TABLE [ANY,STRING]): ANY is
...
feature{NONE} -- Setters
	...
	set_attribute (type: INTEGER; obj: ANY; generic_value: ANY; index: INTEGER) is
	...
	set_string (obj: ANY; value: STRING; index: INTEGER) is
			-- set a string value
		require
...
feature{NONE} -- Schema evolution handling
	...
	evaluate_default_value (type: INTEGER; obj: ANY; i: INTEGER; tmp: TUPLE [LIST [STRING],FUNCTION [ANY,TUPLE,ANY]]) is
			-- evaluate a value
		local
			vars: LIST [STRING]
				-- Names of attributes.
...
feature {NONE} -- Schema evolution handler management.
	...
	handle_object_schema_evolution (type: INTEGER; obj: ANY; field_name: STRING; index, current_version, serialized_version: INTEGER): BOOLEAN is
		...
		local
			...
			tmp: HASH_TABLE[TUPLE[LIST[STRING],FUNCTION[ANY,TUPLE,ANY]],STRING]
...
feature{NONE} --type tests
	...
	is_integer (type: INTEGER): BOOLEAN is
			-- Is `type' an integer?
		do
	...
	is_reference (type: INTEGER): BOOLEAN is
...
feature -- setter
	...
	set_config_path (path: STRING) is
			-- Set the config path.
		do
			config_path := path
		end
