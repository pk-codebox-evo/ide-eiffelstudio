...
%type <INTERVAL_EHR[INTEGER_REF]> integer_interval_value
%type <INTERVAL_EHR[REAL_REF]> real_interval_value
%type <INTERVAL_EHR[ISO8601_TIME]> time_interval_value
%type <INTERVAL_EHR[ISO8601_DATE]> date_interval_value
%type <INTERVAL_EHR[ISO8601_DATE_TIME]> date_time_interval_value
%type <INTERVAL_EHR[ISO8601_DURATION]> duration_interval_value
%type <INTERVAL_EHR[PART_COMPARABLE]> simple_interval_value
	...
	| type_identifier untyped_multiple_attr_object_block
		{
			...
			if $$ /= void then
				...
				complex_object_nodes.item.attributes.last.set_type_name($1)
			end
		}
	;
...
path_value: V_PATH
		{
			create a_path.make_from_string($1)
			$$ := a_path
		}
	| '/'
		{
