						...
						if fld_val /= Void then
							...
							if fld_decoded = Void then
								...
								if equiv_prim_type_id /= 0 then
									...
									if not is_primitive_type (fld_type_id) or fld_val.generating_type.name.starts_with ("STRING") then
						...
						if fld_lst = Void or else fld_lst.has(fld_name) then
							if fld_val /= Void then
								...
								if equiv_prim_type_id /= 0 then
									l_is_shared := false
							-- MP fixed generating_type.name
									if not is_primitive_type (fld_dynamic_type) or fld_val.generating_type.name.starts_with ("STRING") then
										l_path := path_to_shared_obj(fld_val)
										if  l_path /= void then
											l_is_shared := true
											create a_dt_attr.make_single(fld_name)
...
feature {NONE} -- Conversion from decoded object
	...
	create_dt_from_special_decoded_obj(a_dt_attr:DT_ATTRIBUTE_NODE;an_obj:GENERAL_DECODED;a_path:STRING) is
		...
		do
			...
			-- MP fixed generating_type.name
							if not is_primitive_type (dynamic_type (l_item)) or l_item.generating_type.name.starts_with ("STRING") then
				...
				loop
					...
					if l_item /= void then
						if is_any_primitive_conforming_type(dynamic_type (l_item)) then
							...
							--MP refactored to use generating_type.name
							if not is_primitive_type (dynamic_type (l_item)) or l_item.generating_type.name.starts_with ("STRING") then
								l_path := path_to_shared_obj (l_item)
								if l_path /= Void then
									l_is_shared := True
	...
	create_special_base_instance(a_type,a_count:INTEGER):SPECIAL[ANY] is
		...
		do
			...
			when {INTERNAL}.boolean_type then
				-- MP adapted to compile with ES 6.7. I had to use make_filled and provide initialization values
				-- that in my opinion make sense. I tried with make_empty but it crashed (assertion failure)
				-- when trying to put an object in component 0 of a SPECIAL [CHARACTER].
				result := create {SPECIAL[BOOLEAN]}.make_filled (false, a_count) --make (a_count)

			when {INTERNAL}.character_8_type then
				result := create {SPECIAL[CHARACTER_8]}.make_filled (create {CHARACTER_8}, a_count)

			when {INTERNAL}.character_32_type then
				result := create {SPECIAL[CHARACTER_32]}.make_filled (create {CHARACTER_32}, a_count)

			when {INTERNAL}.natural_8_type then
				result := create {SPECIAL[NATURAL_8]}.make_filled (create {NATURAL_8}, a_count)

			when {INTERNAL}.natural_16_type then
				result := create {SPECIAL[NATURAL_16]}.make_filled (create {NATURAL_16}, a_count)

			when {INTERNAL}.natural_32_type then
				result := create {SPECIAL[NATURAL_32]}.make_filled (create {NATURAL_32}, a_count)

			when {INTERNAL}.natural_64_type then
				result := create {SPECIAL[NATURAL_64]}.make_filled (create {NATURAL_64}, a_count)

			when {INTERNAL}.integer_8_type then
				result := create {SPECIAL[INTEGER_8]}.make_filled (create {INTEGER_8}, a_count)

			when {INTERNAL}.integer_16_type then
				result := create {SPECIAL[INTEGER_16]}.make_filled (create {INTEGER_16}, a_count)

			when {INTERNAL}.integer_32_type then
				result := create {SPECIAL[INTEGER_32]}.make_filled (create {INTEGER_32}, a_count)

			when {INTERNAL}.integer_64_type then
				result := create {SPECIAL[INTEGER_64]}.make_filled (create {INTEGER_64}, a_count)

			when {INTERNAL}.real_32_type then
				result := create {SPECIAL[REAL_32]}.make_filled (create {REAL_32}, a_count)

			when {INTERNAL}.real_64_type then
				result := create {SPECIAL[REAL_64]}.make_filled (create {REAL_64}, a_count)

			when {INTERNAL}.pointer_type then
				result := create {SPECIAL[POINTER]}.make_filled (default_pointer, a_count)
