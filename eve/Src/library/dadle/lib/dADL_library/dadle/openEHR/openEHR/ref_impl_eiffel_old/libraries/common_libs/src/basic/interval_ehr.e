indexing
	...
	file:        "$URL: http://www.openehr.org/svn/ref_impl_eiffel/TRUNK/libraries/common_libs/src/basic/interval.e $"
	revision:    "$LastChangedRevision$"
	last_change: "$LastChangedDate$"

class INTERVAL_EHR [G -> PART_COMPARABLE]
...
feature -- Output
	...
	lower_out: STRING is
		...
		do
			...
			if lower.generating_type.name.substring(1,4).is_equal("REAL") and then Result.index_of('.', 1) = 0 then
				Result.append(".0")
			end
		end
	...
	upper_out: STRING is
		...
		do
			...
			Result := upper.out
				-- MP fixed generating_type.name
			if upper.generating_type.name.substring(1,4).is_equal("REAL") and then Result.index_of('.', 1) = 0 then
				Result.append(".0")
			end
		end
