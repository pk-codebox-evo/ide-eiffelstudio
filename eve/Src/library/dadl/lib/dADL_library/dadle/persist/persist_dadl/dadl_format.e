...
feature -- Basic operations
	...
	store (object: ANY; data_file: FILE_MEDIUM)
		...
		do
			...
			create converter
			-- MP: the following if could be removed to pass compilation with ES 6.7 and latest DT_OBJECT_CONVERTER.
			-- feature {DT_OBJECT_CONVERTER}.show_type does not exist anymore
...
--			MP refactored to compile in ES 6.7 and with latest DT_OBJECT_CONVERTER
--			l_complex_node := converter.object_to_dt (object)
...
--						MP refactored to compile in ES 6.7 and with latest DT_OBJECT_CONVERTER
--						The features last_op and fail_reason, suggested in the header comment of class DT_OBJECT_CONVERTER,
--						are not there.
--						if converter.last_op_fail then
--							error_message := converter.fail_reason
--						end
-- 						The following could be a temporary replacement:
--						create error_message.make_from_string("%NRetrieval failure! More information could be obtained from class DT_OBJECT_CONVERTER but is not available.%N")
					end
