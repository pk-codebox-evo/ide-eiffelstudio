...
{	
	...
	private static void printArchetype() {
		if (jAdlInterface.jarchetype_is_valid()) {

			jAdlInterface.jserialise_archetype("adl");
			System.out.println(jAdlInterface.jserialised_archetype());
		}
		else {
			System.out.println(jAdlInterface.jarchetype_errors());
		}
	}
	...
	private static void tc_add_nodes() {
		...
		// 2007 below is the code from AOM OPERATOR_KIND class; should be made a constant definition in a
		// java file of course
		hMatchOp = jAdlInterface.jcreate_expr_binary_operator_node (2007, hIdExprLeaf, hIdPatternExprLeaf);
