indexing
	...
	file:        "$URL: http://www.openehr.org/svn/ref_impl_eiffel/TRUNK/libraries/common_libs/src/structures/data_tree/dt_attribute_node.e $"
...
create
	make_single, make_multiple, make_multiple_generic, make_as_path
...
feature -- Initialisation
	...
	make_as_path(a_path:STRING) is
			-- make as a path holder
...
feature -- Access
	...
	rm_type_name: STRING
			-- type name of this attribute
			-- used to serialize type, if attribute is multiple
...
feature -- Modification
	...
	set_type_name(a_type_name:STRING) is
		...
		local
			l_temp:STRING
		do
			create l_temp.make_from_string (a_type_name)
			l_temp.replace_substring_all ("<", "[")
			l_temp.replace_substring_all (">", "]")
			rm_type_name := l_temp
		end

	set_attr_type(a_type:STRING) is
			-- set attr type
		require
			a_type_valid: a_type /= Void and then not a_type.is_empty
		do
			rm_type_name := create {STRING}.make_from_string (a_type)
		end
	...
	show_type is
			-- show type of attr node when serialized
			-- in case of a multiple attr.
		do
			type_visible := True
		end
