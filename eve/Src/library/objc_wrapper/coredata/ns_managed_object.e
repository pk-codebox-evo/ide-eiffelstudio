...
feature {NONE} -- NSManagedObject Externals
	...
	objc_has_changes (an_item: POINTER): BOOLEAN
		...
		alias
			"[
				return [(NSManagedObject *)$an_item hasChanges];
			 ]"
		end
	...
	objc_changed_values_for_current_event (an_item: POINTER): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <CoreData/CoreData.h>"
		alias
			"[
				return (EIF_POINTER)[(NSManagedObject *)$an_item changedValuesForCurrentEvent];
			 ]"
		end
...
feature -- NSManagedObject
	...
	changed_values_for_current_event: detachable NS_DICTIONARY
			-- Auto generated Objective-C wrapper.
		local
			result_pointer: POINTER
		do
			result_pointer := objc_changed_values_for_current_event (item)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like changed_values_for_current_event} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like changed_values_for_current_event} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
