...
feature {NONE} -- Initialization
	...
	make_with_stream_ (a_stream: detachable NS_INPUT_STREAM)
			-- Initialize `Current'.
		local
		...
		do
			...
			make_with_pointer (objc_init_with_stream_(allocate_object, a_stream__item))
			if item = default_pointer then
				-- TODO: handle initialization error.
			end
		end
...
feature {NONE} -- NSXMLParser Externals
	...
	objc_init_with_stream_ (an_item: POINTER; a_stream: POINTER): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <Foundation/Foundation.h>"
		alias
			"[
				return (EIF_POINTER)[(NSXMLParser *)$an_item initWithStream:$a_stream];
			 ]"
		end
