...
feature -- NSURLPathUtilities
	...
	url_by_appending_path_component__is_directory_ (a_path_component: detachable NS_STRING; a_is_directory: BOOLEAN): detachable NS_URL
			-- Auto generated Objective-C wrapper.
		local
			result_pointer: POINTER
			a_path_component__item: POINTER
		do
			...
			result_pointer := objc_url_by_appending_path_component__is_directory_ (item, a_path_component__item, a_is_directory)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like url_by_appending_path_component__is_directory_} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like url_by_appending_path_component__is_directory_} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
...
feature {NONE} -- NSURLPathUtilities Externals
	...
	objc_url_by_appending_path_component__is_directory_ (an_item: POINTER; a_path_component: POINTER; a_is_directory: BOOLEAN): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <Foundation/Foundation.h>"
		alias
			"[
				return (EIF_POINTER)[(NSURL *)$an_item URLByAppendingPathComponent:$a_path_component isDirectory:$a_is_directory];
			 ]"
		end
