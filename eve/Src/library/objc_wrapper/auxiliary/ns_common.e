...
feature {NONE} -- Removal
	...
	dispose
		...
		do
			if debug_on then
				print ("Release " + get_class_name + " (" + item.out + ")%N")
				log_releasing(item)
			end
		...
		do
			...
			if is_subclass_instance and not l_objc_class.registered then
				...
				across objc_callbacks as objc_callbacks_cursor loop
					...
					if l_objc_class.add_method (selector, callbacks_hijacker, objc_callbacks_cursor.item.objc_encoding) then
					end
					-- Bug: this line is not exectued!
...
--			elseif attached {TYPE [detachable NS_EDGE_INSETS]} eiffel_type then
--				Result.append ("{?=dddd}")
				...
				if arguments_types_count > 0 then
					...
					across arguments_types as cursor loop
						...
						elseif attached {TYPE [detachable AE_DESC]} argument_type then
							...
							arguments_tuple.put (a_struct, arguments_types_index)
							argument.memory_free
--						elseif attached {TYPE [detachable NS_EDGE_INSETS]} argument_type then
--							create {NS_EDGE_INSETS} a_struct.make
--							initialize_ns_edge_insets_struct (argument, a_struct.item)
--							arguments_tuple.put (a_struct, arguments_types_index)
--							argument.memory_free
	...
	initialize_ns_edge_insets_struct (an_item: POINTER; struct_pointer: POINTER)
			-- Auto generated Objective-C wrapper.
		external
			"C inline"
		alias
			"*(NSEdgeInsets *)$struct_pointer = *(NSEdgeInsets *)$an_item"
		end
