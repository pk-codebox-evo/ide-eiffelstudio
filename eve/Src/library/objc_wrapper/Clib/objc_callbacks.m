...
void * pointer_callbacks_hijacker (id self, SEL _cmd, ...) {
	...
	for (i = 2; i < argumentsCount; i++) {
		...
		if (strcmp(argumentType, @encode(NSEdgeInsets)) == 0) {
			NSEdgeInsets *value_pointer = malloc(sizeof(NSEdgeInsets)); // Freeing is done in eiffel code.
			*value_pointer = va_arg(variableArguments, NSEdgeInsets);
			arguments[i - 2] = value_pointer;
			argumentRead = YES;
		}
