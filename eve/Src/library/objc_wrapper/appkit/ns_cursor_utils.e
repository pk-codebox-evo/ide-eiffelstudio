...
feature -- NSCursor
	...
	i_beam_cursor_for_vertical_layout: detachable NS_CURSOR
			-- Auto generated Objective-C wrapper.
		local
			result_pointer: POINTER
			l_objc_class: OBJC_CLASS
		do
			create l_objc_class.make_with_name (get_class_name)
			check l_objc_class_registered: l_objc_class.registered end
			result_pointer := objc_i_beam_cursor_for_vertical_layout (l_objc_class.item)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like i_beam_cursor_for_vertical_layout} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like i_beam_cursor_for_vertical_layout} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
...
feature {NONE} -- NSCursor Externals
	...
	objc_i_beam_cursor_for_vertical_layout (a_class_object: POINTER): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return (EIF_POINTER)[(Class)$a_class_object IBeamCursorForVerticalLayout];
			 ]"
		end
