...
feature -- NSBundleImageExtension
	...
	image_for_resource_ (a_ns_bundle: NS_BUNDLE; a_name: detachable NS_STRING): detachable NS_IMAGE
			-- Auto generated Objective-C wrapper.
		local
			result_pointer: POINTER
			a_name__item: POINTER
		do
			...
			result_pointer := objc_image_for_resource_ (a_ns_bundle.item, a_name__item)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like image_for_resource_} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like image_for_resource_} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
...
feature {NONE} -- NSBundleImageExtension Externals
	...
	objc_image_for_resource_ (an_item: POINTER; a_name: POINTER): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return (EIF_POINTER)[(NSBundle *)$an_item imageForResource:$a_name];
			 ]"
		end
