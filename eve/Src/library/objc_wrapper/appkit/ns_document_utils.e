...
feature -- NSDocument
	...
	autosaves_in_place: BOOLEAN
		...
		do
			create l_objc_class.make_with_name (get_class_name)
			check l_objc_class_registered: l_objc_class.registered end
			Result := objc_autosaves_in_place (l_objc_class.item)
		end

	preserves_versions: BOOLEAN
			-- Auto generated Objective-C wrapper.
		local
			l_objc_class: OBJC_CLASS
		do
			create l_objc_class.make_with_name (get_class_name)
			check l_objc_class_registered: l_objc_class.registered end
			Result := objc_preserves_versions (l_objc_class.item)
		end
...
feature {NONE} -- NSDocument Externals
	...
	objc_autosaves_in_place (a_class_object: POINTER): BOOLEAN
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return [(Class)$a_class_object autosavesInPlace];
			 ]"
		end

	objc_preserves_versions (a_class_object: POINTER): BOOLEAN
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return [(Class)$a_class_object preservesVersions];
			 ]"
		end
...
feature -- NSRestorableState
	...
	restorable_state_key_paths: detachable NS_ARRAY
			-- Auto generated Objective-C wrapper.
		local
			result_pointer: POINTER
			l_objc_class: OBJC_CLASS
		do
			create l_objc_class.make_with_name (get_class_name)
			check l_objc_class_registered: l_objc_class.registered end
			result_pointer := objc_restorable_state_key_paths (l_objc_class.item)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like restorable_state_key_paths} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like restorable_state_key_paths} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
...
feature {NONE} -- NSRestorableState Externals
	...
	objc_restorable_state_key_paths (a_class_object: POINTER): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return (EIF_POINTER)[(Class)$a_class_object restorableStateKeyPaths];
			 ]"
		end
