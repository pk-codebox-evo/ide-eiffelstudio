...
feature -- NSConstraintBasedCompatibility
	...
	requires_constraint_based_layout: BOOLEAN
			-- Auto generated Objective-C wrapper.
		local
			l_objc_class: OBJC_CLASS
		do
			create l_objc_class.make_with_name (get_class_name)
			check l_objc_class_registered: l_objc_class.registered end
			Result := objc_requires_constraint_based_layout (l_objc_class.item)
		end

feature {NONE} -- NSConstraintBasedCompatibility Externals

	objc_requires_constraint_based_layout (a_class_object: POINTER): BOOLEAN
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return [(Class)$a_class_object requiresConstraintBasedLayout];
			 ]"
		end
