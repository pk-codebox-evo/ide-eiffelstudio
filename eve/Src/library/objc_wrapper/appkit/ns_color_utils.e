...
--		end
	...
	color_with_generic_gamma22_white__alpha_ (a_white: REAL_64; a_alpha: REAL_64): detachable NS_COLOR
			-- Auto generated Objective-C wrapper.
		local
			result_pointer: POINTER
			l_objc_class: OBJC_CLASS
		do
			create l_objc_class.make_with_name (get_class_name)
			check l_objc_class_registered: l_objc_class.registered end
			result_pointer := objc_color_with_generic_gamma22_white__alpha_ (l_objc_class.item, a_white, a_alpha)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like color_with_generic_gamma22_white__alpha_} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like color_with_generic_gamma22_white__alpha_} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
	...
	color_with_srgb_red__green__blue__alpha_ (a_red: REAL_64; a_green: REAL_64; a_blue: REAL_64; a_alpha: REAL_64): detachable NS_COLOR
			-- Auto generated Objective-C wrapper.
		local
			result_pointer: POINTER
			l_objc_class: OBJC_CLASS
		do
			create l_objc_class.make_with_name (get_class_name)
			check l_objc_class_registered: l_objc_class.registered end
			result_pointer := objc_color_with_srgb_red__green__blue__alpha_ (l_objc_class.item, a_red, a_green, a_blue, a_alpha)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like color_with_srgb_red__green__blue__alpha_} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like color_with_srgb_red__green__blue__alpha_} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
...
--		end
	...
	objc_color_with_generic_gamma22_white__alpha_ (a_class_object: POINTER; a_white: REAL_64; a_alpha: REAL_64): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return (EIF_POINTER)[(Class)$a_class_object colorWithGenericGamma22White:$a_white alpha:$a_alpha];
			 ]"
		end

	objc_color_with_srgb_red__green__blue__alpha_ (a_class_object: POINTER; a_red: REAL_64; a_green: REAL_64; a_blue: REAL_64; a_alpha: REAL_64): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return (EIF_POINTER)[(Class)$a_class_object colorWithSRGBRed:$a_red green:$a_green blue:$a_blue alpha:$a_alpha];
			 ]"
		end
