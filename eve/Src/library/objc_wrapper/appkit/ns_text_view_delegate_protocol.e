...
--		end
	...
	text_view__url_for_contents_of_text_attachment__at_index_ (a_text_view: detachable NS_TEXT_VIEW; a_text_attachment: detachable NS_TEXT_ATTACHMENT; a_char_index: NATURAL_64): detachable NS_URL
			-- Auto generated Objective-C wrapper.
		require
			has_text_view__url_for_contents_of_text_attachment__at_index_: has_text_view__url_for_contents_of_text_attachment__at_index_
		local
			result_pointer: POINTER
			a_text_view__item: POINTER
			a_text_attachment__item: POINTER
		do
			if attached a_text_view as a_text_view_attached then
				a_text_view__item := a_text_view_attached.item
			end
			if attached a_text_attachment as a_text_attachment_attached then
				a_text_attachment__item := a_text_attachment_attached.item
			end
			result_pointer := objc_text_view__url_for_contents_of_text_attachment__at_index_ (item, a_text_view__item, a_text_attachment__item, a_char_index)
			if result_pointer /= default_pointer then
				if attached objc_get_eiffel_object (result_pointer) as existing_eiffel_object then
					check attached {like text_view__url_for_contents_of_text_attachment__at_index_} existing_eiffel_object as valid_result then
						Result := valid_result
					end
				else
					check attached {like text_view__url_for_contents_of_text_attachment__at_index_} new_eiffel_object (result_pointer, True) as valid_result_pointer then
						Result := valid_result_pointer
					end
				end
			end
...
--		end
	...
	has_text_view__url_for_contents_of_text_attachment__at_index_: BOOLEAN
			-- Auto generated Objective-C wrapper.
		do
			Result := objc_has_text_view__url_for_contents_of_text_attachment__at_index_ (item)
		end
...
--		end
	...
	objc_has_text_view__url_for_contents_of_text_attachment__at_index_ (an_item: POINTER): BOOLEAN
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return [(id)$an_item respondsToSelector:@selector(textView:URLForContentsOfTextAttachment:atIndex:)];
			 ]"
		end
...
--		end
	...
	objc_text_view__url_for_contents_of_text_attachment__at_index_ (an_item: POINTER; a_text_view: POINTER; a_text_attachment: POINTER; a_char_index: NATURAL_64): POINTER
			-- Auto generated Objective-C wrapper.
		external
			"C inline use <AppKit/AppKit.h>"
		alias
			"[
				return (EIF_POINTER)[(id <NSTextViewDelegate>)$an_item textView:$a_text_view URLForContentsOfTextAttachment:$a_text_attachment atIndex:$a_char_index];
			 ]"
		end
