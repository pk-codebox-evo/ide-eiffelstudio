...
note
	...
	file:        "$URL: http://www.openehr.org/svn/ref_impl_eiffel/BRANCHES/adl1.5/libraries/common_libs/src/structures/syntax/dadl/parser/dadl2_validator.y $"
...
object_block: complex_object_block
	...
	| SYM_START_DBLOCK SYM_END_DBLOCK -- effectively ignore empty objects
	;
...
type_identifier: '(' V_TYPE_IDENTIFIER ')'
	...
	| '(' V_GENERIC_TYPE_IDENTIFIER ')'
		{
			$$ := $2
			type_name := uml_to_eiffel_typename ($2)
		}
	;
...
numeric_value: V_INTEGER
		{
			if not attached type_name then
				$$ := $1.to_integer

			-- signed integer types
			elseif type_name.is_equal(Eiffel_signed_integer_8_type_name) then
				$$ := $1.to_integer_8
			elseif type_name.is_equal(Eiffel_signed_integer_16_type_name) then
				$$ := $1.to_integer_16
			elseif type_name.is_equal(Eiffel_signed_integer_32_type_name) then
				$$ := $1.to_integer_32
			elseif type_name.is_equal(Eiffel_signed_integer_64_type_name) then
				$$ := $1.to_integer_64

			-- unsigned integer types
			elseif type_name.is_equal(Eiffel_signed_natural_8_type_name) then
				$$ := $1.to_natural_8
			elseif type_name.is_equal(Eiffel_signed_natural_16_type_name) then
				$$ := $1.to_natural_16
			elseif type_name.is_equal(Eiffel_signed_natural_32_type_name) then
				$$ := $1.to_natural_32
			elseif type_name.is_equal(Eiffel_signed_natural_64_type_name) then
				$$ := $1.to_natural_64
			end
	...
	| V_REAL
		{
			if not attached type_name then
				$$ := $1.to_real
			elseif type_name.is_equal(Eiffel_float_32_type_name) or type_name.is_equal(Eiffel_real_type_name) then
				$$ := $1.to_real
			elseif type_name.is_equal(Eiffel_float_64_type_name) or type_name.is_equal(Eiffel_double_type_name) then
				$$ := $1.to_double
			end
		}
	;
