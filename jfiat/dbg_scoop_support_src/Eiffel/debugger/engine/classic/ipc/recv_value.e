...
feature	{} -- Initialization of the C/Eiffel interface
		...
		do
			...
			if Eiffel_system.valid_dynamic_id (l_type_id) then
				...
				if cl /= Void and then cl.is_special then
					create spe.make_set_ref (add, l_type_id)
					spe.set_scoop_processor_id (scp_pid)
					item := spe
				else
					create ref_value.make (add, l_type_id)
					ref_value.set_scoop_processor_id (scp_pid)
					item := ref_value
				end
			else
	...
	set_exception_ref (ref: POINTER; type: INTEGER; scp_pid: NATURAL_16)
		...
		do
			...
			create add.make_from_pointer (ref)
			create rf.make (add, type + 1)
			rf.set_scoop_processor_id (scp_pid)
