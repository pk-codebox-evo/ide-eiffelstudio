		...
		if (pack.rq_type == DUMPED) {
			switch (pack.rq_dump.dmp_type) {
				case DMP_EXCEPTION_ITEM:
					...
					set_exception_ref (eif_access (target), item.it_ref, item.type & SK_DTYPE, RTS_PID(item.it_ref));
					return;
				case DMP_ITEM:
					item = *pack.rq_dump.dmp_item;
					type_flag = item.type;
					switch (type_flag & SK_HEAD) {
						...
						case SK_EXP:
							...
							set_ref (eif_access (target), item.it_ref, type_flag & SK_DTYPE, RTS_PID(item.it_ref));
							return;
						case SK_BIT:
