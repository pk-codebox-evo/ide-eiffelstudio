...
feature -- Data
	...
	data_from_storage: STRING_32
			-- Repository data loaded from storage
		local
			l_session: SESSION_I
		do
			l_session := session_data
			Result ?= l_session.value (Repositories_session_data_id)
		end
		...
		do
			...
			if cons.is_service_available then
				...
				l_session := session_data
					...
					--| Set data
				l_session.set_value (a_data, Repositories_session_data_id)
...
feature {NONE} -- session data access
	...
	Repositories_session_data_id: STRING = "repositories"
