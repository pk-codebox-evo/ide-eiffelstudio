...
inherit
	...
	ROUTINE [BASE_TYPE, OPEN_ARGS]
		rename
			is_equal as routine_is_equal,
			copy as routine_copy
		export
			{NONE} all
		end
...
convert
	proxy_: {
			SCOOP_SEPARATE__FUNCTION[BASE_TYPE, OPEN_ARGS, RESULT_TYPE],
...
feature -- Comparison
	...
	is_equal (other: like Current): BOOLEAN
		...
		do
			Result := routine_is_equal (other) and then (last_result ~ other.last_result)
		end
...
feature -- SCOOP
	with_processor_(a_processor: SCOOP_PROCESSOR) : like Current
		do
			set_processor_(a_processor)
			create proxy_.make(a_processor, Current)
			Result := Current
		end

feature -- Proxy
	proxy_ : SCOOP_SEPARATE__FUNCTION[BASE_TYPE, OPEN_ARGS, RESULT_TYPE]
