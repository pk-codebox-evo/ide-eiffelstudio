...
feature -- json
	...
	json: JSON_FORMAT
		once
			...
			json_not_void: Result /= Void
		end

feature -- Display

	trace (a_error: ERROR; a_kind: INTEGER)
...
feature -- Processing
	...
	process_syntax_error (a_value: SYNTAX_ERROR)
		...
		do
			...
			if type = normal then
				...
				json.set_error_msg ("Syntax error")
				json.set_line_number(a_value.line)
					-- Error happened in a class
				if attached {CLASS_C} a_value.associated_class as l_class1 then
					json.set_class_name (l_class1.name)
				elseif a_value.file_name /= Void then
						-- `associated_class' May be void at degree 6 when parsing partial classes
					json.set_file_name (a_value.file_name.to_string_8)
				end
				if a_value.error_message /= Void and then not a_value.error_message.is_empty then
					json.set_error_msg (a_value.error_message)
				end
				l_msg := a_value.syntax_message
				if not l_msg.is_empty then
					json.set_syntax_msg (l_msg)
				end
				if a_value.has_source_text then
					create helper_string.make_from_string(a_value.current_line_32)
					from
						i := 1
					until
						i > helper_string.count or (helper_string.item (i) /= '%T' and helper_string.item (i) /= ' ')
					loop
						i := i + 1
					end
					helper_string.remove_head (i-1)
					json.set_source_code (helper_string)
				else
					json.set_source_code ("source code is not available")
				end
	...
	process_syntax_warning (a_value: SYNTAX_WARNING)
		...
		do
			if type = normal then
				...
				json.set_error_msg ("Obsolete syntax used")
				json.set_line_number (a_value.line)
					-- Error happened in a class
				if attached {CLASS_C} a_value.associated_class as l_class1 then
					json.set_class_name (l_class1.name)
	...
	process_user_defined_error (a_value: USER_DEFINED_ERROR)
			-- Process object `a_value'.
		local
			l_text: LIST[STRING_8]
			helper_string: STRING_8
			helper_locals: STRING_8
			l_name: STRING
			l_type: TYPE_A
		do
			if attached {COMPILER_ERROR} a_value as l_compiler_error then
				...
				when normal then
					...
					l_text := l_compiler_error.help_text
						if not l_text.is_empty then
							from l_text.start until l_text.after loop
								if not l_text.is_empty then
									helper_string.append (l_text.item + " ")
								end
								l_text.forth
							end
						else
							helper_string.append ("No help available for this error (Cannot read file: ")
							helper_string.append (l_compiler_error.help_file_name)
							helper_string.append (")");
							helper_string.append (", ")
							helper_string.append ("An error message should always be available. Please contact Eiffel Software.");

						end
					if attached {EIFFEL_WARNING} l_compiler_error as json_warning then
						json.set_warning_msg (helper_string)
						json.set_class_name (json_warning.associated_class.name)
						if attached {UNUSED_LOCAL_WARNING} l_compiler_error as json_warning2 then
							json.set_feature_name (json_warning2.associated_feature.name_8)
							create helper_locals.make_empty
							from
								json_warning2.unused_locals.start
							until
								json_warning2.unused_locals.after
							loop
								l_name := json_warning2.unused_locals.item.name
								l_type := json_warning2.unused_locals.item.type
								check
									l_name_not_void: l_name /= Void
									l_type_not_void: l_type /= Void
								end
								helper_locals.append (l_name + ": " + l_type.name + ", ")
								json_warning2.unused_locals.forth
							end
							helper_locals.remove_tail (2)
							json.set_unused_local (helper_locals)


						end
						if attached {OBS_CLASS_WARN} l_compiler_error as json_warning3 then
							json.set_feature_name (json_warning3.associated_feature.name_8)
						end


					else
						json.set_error_msg (helper_string)
						if l_compiler_error.line /= 0 then
							json.set_line_number (l_compiler_error.line)
						end
					end

					if attached {EIFFEL_ERROR} l_compiler_error as json_error_class then
						json.set_class_name (json_error_class.class_c.name)
						if attached {FEATURE_ERROR} l_compiler_error as json_error_feature then
							json.set_feature_name (json_error_feature.e_feature.name_32)
							if attached {VEEN} l_compiler_error as json_error_identifier then
								json.set_identifier_name (json_error_identifier.identifier)
							end
						end
					end
	...
	process_viin (a_value: VIIN)
		...
		do
			if type = normal then
				...
				print_short_help_json (a_value)
				a_value.initialize_output
				if attached {CLASS_C} a_value.associated_class as l_class then
					json.set_class_name (l_class.name)
				elseif a_value.file_name /= Void then
...
feature {NONE} -- Implementation
	...
	frozen print_short_help_json (a_error: ERROR)
			-- Display help
		require
			error_not_void: a_error /= Void
		local
		...
		do
			...
			if file.exists then
				...
				loop
					file.read_line
					json.error_msg.append (file.last_string.twin)
				end
				file.close
			else
				json.error_msg.append ("No help available for this error ")
				json.error_msg.append ("(cannot read file: ")
				json.error_msg.append (f_name.name.to_string_8)
				json.error_msg.append ("). ")
				json.error_msg.append ("An error message should always be available. ")
				json.error_msg.append ("Please contact ISE.")
			end
		end
