...
feature -- tools
	...
	cdd_manager: CDD_MANAGER is
			-- Manager for generating test cases
		once
			create Result.make
		end
...
--				Window_manager.routine_win_mgr.synchronize_with_callstack
	...
	on_application_just_stopped is
		...
		do
			...
			-- Notify cdd manager that the application stopped
			cdd_manager.on_application_stopped (Application.status, debugging_window)
	...
	on_application_quit is
		...
		do
			...
			if Application /= Void and then Application.is_running then
				...
				cdd_manager.on_application_quit
			end
			debug ("debugger_trace_synchro")
