-- This is the .NET version of SCOOP_SCHEDULER
indexing
...
feature {SCOOP_SEPARATE_CLIENT} -- Basic operations
	...
	execute_routine (a_caller_: SCOOP_SEPARATE_CLIENT; a_requested_processors: TUPLE [SCOOP_PROCESSOR]; a_routine: ROUTINE [SCOOP_SEPARATE_TYPE, TUPLE]; a_wait_condition: FUNCTION [SCOOP_SEPARATE_CLIENT, TUPLE, BOOLEAN]) is
...
feature {NONE} -- Implementation
	...
	find_and_set_processor_ (a_caller_: SCOOP_SEPARATE_CLIENT) is
		...
		local
			a_caller_thread_id: INTEGER --POINTER
		do
			a_caller_thread_id := current_thread_id -- thread_id --get_current_id
			from
				processors_mutex.lock
				processors.start
			until
