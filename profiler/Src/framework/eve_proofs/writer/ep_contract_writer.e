	...
	generate_contracts
		...
		do
			...
			elseif not current_feature.written_class.is_generic then
				...
				Context.init (current_feature.written_class.actual_type.associated_class_type (Void))
				--check False end
			end
			Context.set_current_feature (current_feature)

			check Context.class_type /= Void end
...
-- MML test
					...
					if l_assert.tag /= Void and then l_assert.tag.starts_with ("mml") then
						mml_writer.reset
						mml_writer.set_context_target_type (current_feature.written_class.actual_type)
						...
						-- TODO: may need to use current_generic_type
...
-- MML test
					if l_assert.tag /= Void and then l_assert.tag.starts_with ("mml") then
						...
						expression_writer.reset
						expression_writer.set_context_target_type (a_class.actual_type)
						expression_writer.set_processing_contract
						l_assert.expr.process (expression_writer)
						expression_writer.set_not_processing_contract
