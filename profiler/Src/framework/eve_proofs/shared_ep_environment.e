...
-- TODO: add 'indent' 'unindent' features
	...
	string_value_in_feature_indexing (a_feature: FEATURE_I; a_tag: STRING; a_default: STRING): STRING
			-- Value of first tag `a_tag' in indexing of `a_feature', or `a_default' if tag is not set
		local
			l_indexing_clause: INDEXING_CLAUSE_AS
		do
			l_indexing_clause := a_feature.written_class.ast.feature_with_name (a_feature.feature_name_id).indexes
			Result := string_value_in_indexing (l_indexing_clause, a_tag, a_default)
		end

	boolean_value_in_class_indexing (a_class: !CLASS_C; a_tag: STRING; a_default: BOOLEAN): BOOLEAN
			-- Value of first tag `a_tag' in indexing of `a_class', or `a_default' if tag is not set
		local
			l_indexing_clause: INDEXING_CLAUSE_AS
		do
	...
	string_value_in_class_indexing (a_class: !CLASS_C; a_tag: STRING; a_default: STRING): STRING
			-- Value of first tag `a_tag' in indexing of `a_class', or `a_default' if tag is not set
		local
			l_indexing_clause: INDEXING_CLAUSE_AS
		do
		...
		do
			...
			if a_indexing_clause /= Void then
				...
				loop
					...
					if l_index.tag.name_8.as_lower.is_equal (a_tag.as_lower) then
						l_found := True
		...
		do
			...
			if a_indexing_clause /= Void then
				...
				loop
					...
					if l_index.tag.name_8.as_lower.is_equal ("proof") then
						l_found := True
						l_bool ?= l_index.index_list.first
						if l_bool /= Void then
							Result := l_bool.value
	...
	string_value_in_indexing (a_indexing_clause: INDEXING_CLAUSE_AS; a_tag: STRING; a_default: STRING): STRING
			-- Value of first tag `a_tag' in indexing of `a_feature', or `a_default' if tag is not set
		local
			l_index: INDEX_AS
			l_string: STRING_AS
		...
		do
			...
			if a_indexing_clause /= Void then
				...
				loop
					...
					if l_index.tag.name_8.as_lower.is_equal (a_tag.as_lower) then
						l_found := True
						l_string ?= l_index.index_list.first
						if l_string /= Void then
							Result := l_string.value
						end
					end
					a_indexing_clause.forth
				end
