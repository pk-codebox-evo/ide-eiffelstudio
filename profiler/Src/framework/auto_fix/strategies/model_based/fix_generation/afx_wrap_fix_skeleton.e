...
feature{NONE} -- Implementation
	...
	generate_fixes_from_snippet (a_snippets: LINKED_LIST [TUPLE [snippet: STRING_8; ranking: INTEGER_32]]; a_precondition: EPA_STATE; a_postcondition: EPA_STATE; a_ignore_state_change: BOOLEAN)
		...
		do
			...
			fixes.force_last (fix_with_text (Void, 0, a_precondition, a_postcondition))
			...
			if not a_ignore_state_change and then a_snippets /= Void then
				...
				loop
					...
					if not l_fix_text.is_empty then
						fixes.force_last (fix_with_text ("%N" + l_fix_text + "%N", a_snippets.item_for_iteration.ranking, a_precondition, a_postcondition))
					end
					a_snippets.forth
				end
			end
