...
feature -- Status report
	...
	has_first: BOOLEAN is
			--
		do
			Result := first /= Void
		end

	has_second:BOOLEAN is
			--
		do
			Result := second /= Void
		end

	rule_count: INTEGER is
			--
		do
			if rules /= void then
				Result := rules.count
...
feature -- Status setting
	...
	set_first (n: STRING; s: DS_HASH_TABLE[STRING, STRING]) is
			--
		do
			first_name := n
			first := s
	...
	set_second (n: STRING; s: DS_HASH_TABLE[STRING, STRING]) is
			--
		do
			second_name := n
			second := s
...
feature {NONE} -- Implementation
		...
		do
			from second.start until second.after loop
				...
				if first.has (feature_name) then -- has same attribute
					...
					r.set_new_type (feature_class)
					r.add_comment ("What is the default value?")
					r.add_comment ("Please also check the class invariant!")
				end
				add_rule (r)
				second.forth
			end
	...
	text_append_line_tabs(s: STRING; t: INTEGER) is
