...
feature -- Code generation
	...
	filter_code (class_name: STRING; default_fields_value: DS_HASH_TABLE [STRING, STRING]): STRING is
...
feature -- Code instrumentation
	...
	add_inheritance_clause_from_filtered_class (source_code, class_name: STRING): STRING is
...
feature -- Handlers management
	...
	update_main_handler_code (source_code_input, class_name: STRING): STRING is
	...
	update_class_handler_code (source_code: STRING; ver1,ver2: INTEGER; attribute_list_v1, attribute_list_v2: DS_HASH_TABLE [STRING, STRING]): STRING is
		...
		do
			...
			from lines.start until lines.after loop
				...
				if not is_creation_feature_inserted and then line.has_substring ("Precursor {" + const.Schema_evolution_handler_class + "}") then
					Result.append ("%T%Tset_convertion_function (" + ver1.out + "," + ver2.out + "," + function_name (ver1, ver2) + ")%N")
	...
	class_handler_code (ver1,ver2: INTEGER; class_name: STRING; attribute_list_v1, attribute_list_v2: DS_HASH_TABLE [STRING, STRING]): STRING is
		...
		do
			...
			class_name.to_upper
			Result := "indexing%N" +
			"%Tdescription: %"Patch for the " + class_name + " class%"%N%N" +
			"class%N" +
...
feature {NONE} --Implementation
	...
	conversion_function (ver1, ver2: INTEGER; attribute_list_v1, attribute_list_v2: DS_HASH_TABLE [STRING, STRING]): STRING is
	...
	converter (ver1,ver2: INTEGER; attribute_list_v1, attribute_list_v2: DS_HASH_TABLE [STRING, STRING]): STRING is
	...
	function_name (ver1,ver2: INTEGER): STRING is
			-- The function name.
		do
			Result := "v" + ver1.out + "_to_v" + ver2.out
		end

	get_main_handler_body: STRING is
	...
	filter_content (class_name_to_filter:STRING; default_fields_value: DS_HASH_TABLE[STRING,STRING]): STRING is
			-- The filter body.
		require
