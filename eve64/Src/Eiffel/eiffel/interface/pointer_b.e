...
feature {CLASS_TYPE_AS} -- Actual class type
	...
	partial_actual_type (gen: ARRAY [TYPE_A]; is_exp: BOOLEAN; is_sep: BOOLEAN; proc_tag_type : PROCESSOR_TAG_TYPE): CL_TYPE_A is
		...
		do
			if is_typed_pointer then
				...
				Result := Precursor {CLASS_B} (gen, is_exp, is_sep, proc_tag_type)
			end
		end
