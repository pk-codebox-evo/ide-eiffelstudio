...
class AUT_DETERMINISTIC_INPUT_CREATOR
...
feature -- Access
	...
	object_creator: AUT_DETERMINISTIC_OBJECT_CREATOR
    ...
    typesNumber : HASH_TABLE [INTEGER,TYPE_A]
	...
	step
		...
		do
			...
			elseif receivers.count < types.count then
				...
				if (typesnumber.has (type)) then
				    n_objects := typesnumber.item (type)
				else
				   typesnumber.put (parameter_loader.get_next_object_creation_number , type)
				   n_objects := typesnumber.item (type)
				end

				n_variables := interpreter.variable_table.conforming_variables (interpreter_root_class,type).count


				if n_variables <  n_objects or types.item (receivers.count + 1).is_expanded then
					create_object_creator
				else
					if parameter_loader.is_evolving_method_call then
                        list :=  interpreter.variable_table.conforming_variables (interpreter_root_class, types.item (receivers.count + 1))
		...
		do
			...
			if not type.is_expanded then
				if creation_procedure_count (type, system) = 0 then
					...
					type := deterministic_creatable_descendant (type)
				end
			end
			if type = Void then
...
--				receiver := interpreter.variable_table.random_conforming_variable (interpreter_root_class, types.item (receivers.count + 1))
	...
	deterministic_creatable_descendant (a_type: TYPE_A): TYPE_A
			-- Create a list with possibla types and pick one according to the chromosome
		require
			a_type_not_void: a_type /= Void
		local
			class_: CLASS_C
