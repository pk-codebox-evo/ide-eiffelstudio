indexing
	...
	file:        "$URL: http://www.openehr.org/svn/ref_impl_eiffel/TRUNK/libraries/common_libs/src/structures/syntax/dadl/serialiser/dadl_syntax_serialiser.e $"
...
feature -- Visitor
	...
	end_complex_object_node(a_node: DT_COMPLEX_OBJECT_NODE; depth: INTEGER) is
		...
		do
			a_node.start
			if a_node.attributes.count > 0 and then a_node.item.is_path then
	...
	start_attribute_node(a_node: DT_ATTRIBUTE_NODE; depth: INTEGER) is
		...
		do
			if not a_node.is_generic then
				...
				if a_node.is_path then
					last_result.right_adjust
					last_result.append(apply_style(a_node.rm_attr_name, STYLE_IDENTIFIER))
				else
					last_result.append(create_indent(depth//2 + multiple_attr_count) +
					...
					if a_node.is_multiple then
						...
						if a_node.is_typed and a_node.type_visible then
							last_result.append(convert_dadl_type_name(a_node.rm_type_name) + format_item(FMT_SPACE))
						end
						last_result.append(symbol(SYM_START_DBLOCK) + format_item(FMT_NEWLINE))
					end
				end
