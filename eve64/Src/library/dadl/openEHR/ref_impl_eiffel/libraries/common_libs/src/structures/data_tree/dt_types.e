indexing
	...
	file:        "$URL: http://www.openehr.org/svn/ref_impl_eiffel/TRUNK/libraries/common_libs/src/structures/data_tree/dt_types.e $"
...
feature {NONE} -- Definitions
	...
	primitive_types: ARRAYED_LIST[INTEGER] is
		once
			...
			Result.extend (dynamic_type_from_string ("INTEGER_8"))
			Result.extend (dynamic_type_from_string ("INTEGER_8_REF"))
			Result.extend (dynamic_type_from_string ("INTEGER_16"))
			Result.extend (dynamic_type_from_string ("INTEGER_16_REF"))
			Result.extend (dynamic_type_from_string ("INTEGER_32"))
			Result.extend (dynamic_type_from_string ("INTEGER_32_REF"))
			Result.extend (dynamic_type_from_string ("INTEGER_64"))
			Result.extend (dynamic_type_from_string ("INTEGER_64_REF"))
			Result.extend (dynamic_type_from_string ("NATURAL_8"))
			Result.extend (dynamic_type_from_string ("NATURAL_16"))
			Result.extend (dynamic_type_from_string ("NATURAL_32"))
			Result.extend (dynamic_type_from_string ("NATURAL_64"))
			Result.extend (dynamic_type_from_string ("NATURAL_32_REF"))
			Result.extend (dynamic_type_from_string ("NATURAL_16_REF"))
			Result.extend (dynamic_type_from_string ("NATURAL_8_REF"))
			Result.extend (dynamic_type_from_string ("NATURAL_64_REF"))
			Result.extend (dynamic_type_from_string ("NATURAL"))
			Result.extend (dynamic_type_from_string ("REAL_32"))
			Result.extend (dynamic_type_from_string ("REAL_32_REF"))
			Result.extend (dynamic_type_from_string ("REAL_64_REF"))
			...
			Result.extend (dynamic_type_from_string ("CHARACTER_8_REF"))
			Result.extend (dynamic_type_from_string ("CHARACTER_32_REF"))
			Result.extend (dynamic_type_from_string ("CHARACTER_32"))

			Result.extend (dynamic_type_from_string("POINTER"))
			Result.extend (dynamic_type_from_string("POINTER_REF"))
		end
	...
	primitive_interval_types: ARRAYED_LIST [INTEGER] is
		...
		once
			Create Result.make (0)
			Result.compare_objects
			Result.extend (dynamic_type (create {INTERVAL_EHR [INTEGER]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [REAL]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [DOUBLE]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [DATE]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [DATE_TIME]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [TIME]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [DATE_TIME_DURATION]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [INTEGER_REF]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [REAL_REF]}))
			Result.extend (dynamic_type (create {INTERVAL_EHR [DOUBLE_REF]}))
		end
...
feature -- Status Report
	...
	is_container_type(a_type_id: INTEGER): BOOLEAN is
			...
			-- or ARRAY ?! added by lucien hansen
		do
			debug ("DT")
				io.put_string(generator +
			...
			Result := type_conforms_to(a_type_id, sequence_any_type_id) or
				type_conforms_to(a_type_id, hash_table_any_hashable_type_id) or type_conforms_to(a_type_id, array_any_type_id)
			debug ("DT")
...
feature {NONE} -- Implementation
	...
	array_any_type_id:INTEGER is
			-- dynamic type of ARRAY[ANY]
		once
			Result := dynamic_type_from_string("ARRAY[ANY]")
		end
