indexing
	...
	file:        "$URL: http://www.openehr.org/svn/ref_impl_eiffel/TRUNK/libraries/common_libs/src/structures/object_graph/path/og_path.e $"
...
feature -- Initialisation
	...
	make_from_string(s: STRING) is
		...
		do
			...
			--FIXME: parser is wrong: generic attribute name "_items" is parsed as "items" !
			-- manually re-adjust
			create l_dt_attr.make_single ("dummy")
			if s.has_substring (l_dt_attr.representation.generic_attr_name) then
				from
					start
				until
					off or item.is_feature_call
				loop
					if item.attr_name.is_equal ("items") then
						item.set_attr_name(l_dt_attr.representation.generic_attr_name)
					end
					forth
				end
			end
