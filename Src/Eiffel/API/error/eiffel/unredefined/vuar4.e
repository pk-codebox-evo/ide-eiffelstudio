indexing
	...
	description:
		"Error for a feature call: type mismatch on one argument: like Current not allowed on non monomorphic"
	...
	is_defined: BOOLEAN is
		...
		do
			Result := Precursor and then
				argument_name /= Void and then target_type /= Void and then formal_type /= Void and then actual_type /= Void
		end
...
feature {COMPILER_EXPORTER} -- Setting
	...
	set_target_type (a: TYPE_A) is
			-- Assign `a' to `target_type'.
		require
			a_not_void: a /= Void
		do
			target_type := a;
		ensure
			is_set: target_type = a
		end
