	...
	set_stone_after_first_check (a_stone: STONE) is
		...
		do
			...
			editor := develop_window.editors_manager.editor_with_stone (a_stone)
				-- Locate editor
			if not develop_window.is_history_moving then
				if editor /= Void then
			...
			then
				...
				then
					if develop_window.is_history_moving then
						develop_window.locate_editor_stoned (new_class_stone)
					end
				...
				then
					if develop_window.is_history_moving then
						develop_window.locate_editor_stoned (cluster_st)
					end
			...
			if old_class_stone /= Void then
				...
				if not develop_window.is_history_moving and then same_class and then feature_stone /= Void then
					same_class := False
			...
			if develop_window.editors_manager.current_editor /= Void and then not develop_window.editors_manager.current_editor.has_focus then
				develop_window.ev_application.idle_actions.extend_kamikaze (agent (develop_window.editors_manager.current_editor).set_focus)
			end
		end
