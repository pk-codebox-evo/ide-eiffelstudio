	...
	prepare (a_stone: STONE) is
		...
		do
			...
			if a_stone /= Void then
				a_stone.set_pos_editor (develop_window.editors_manager.current_editor)
			end
		end
	...
	handle_break_error_ace_external_file_stone (a_stone: STONE) is
		...
		do
			if conv_brkstone /= Void then
				app_exec := develop_window.Debugger_manager.application
	...
	handle_cluster_stone is
		...
		do
			...
			if cluster_st /= Void then
				...
				if cluster_st.is_cluster and then not text_loaded then
...
--| END FIXME
	...
	formatted_context_for_cluster (a_cluster: CLUSTER_I; a_path: STRING) is
		...
		do
			check
				current_editor_not_void: current_editor /= Void
			end
			create l_format_context.make_for_case (develop_window.editors_manager.current_editor.text_displayed)
			develop_window.editors_manager.current_editor.handle_before_processing (False)
			...
			l_format_context.exdent
			l_format_context.put_new_line
			develop_window.editors_manager.current_editor.handle_after_processing
		end
