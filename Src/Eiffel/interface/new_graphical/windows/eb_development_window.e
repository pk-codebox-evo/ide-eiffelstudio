...
inherit
	EB_TOOL_MANAGER
		...
		export
			{EB_STONE_CHECKER, EB_DEVELOPMENT_WINDOW_BUILDER, EB_DEVELOPMENT_WINDOW_PART}
				Warning_messages, Interface_names,
				init_commands, Pixmaps,
				general_customizable_toolbar
...
feature -- Update
	...
	synchronize is
		...
		do
			...
			if has_case then
				tools.metric_tool.set_recompiled (True)
			end
	...
	toggle_unified_stone is
		...
		do
			...
			if unified_stone then
				...
				-- To be implemented.
--				tools.context_tool.link_to_development_window
			else
				if stone /= Void then
					commands.send_stone_to_context_cmd.enable_sensitive
				end
				-- To be implemented.
--				tools.context_tool.cut_from_development_window
			end
		end
	...
	force_stone (s: STONE) is
		...
		do
			if s.is_storable then
				...
				if not unified_stone then
					-- To be implemented.
--					tools.context_tool.advanced_set_stone (s)
				end
			end
		end
...
feature -- Window management
	...
	save_editors_to_session_data (a_window_data: EB_DEVELOPMENT_WINDOW_SESSION_DATA) is
		...
		do
			a_window_data.save_open_classes (editors_manager.open_classes)
	...
	internal_construct_standard_layout_by_code is
		...
		do
			...
			-- Tool bars
			l_tool_bar_content := docking_manager.tool_bar_manager.content_by_title (interface_names.m_general_toolbar)
			check not_void: l_tool_bar_content /= Void end
			l_tool_bar_content.set_top ({SD_ENUMERATION}.top)

			l_tool_bar_content := docking_manager.tool_bar_manager.content_by_title (interface_names.m_address_toolbar)
			check not_void: l_tool_bar_content /= Void end
			l_tool_bar_content.set_top ({SD_ENUMERATION}.top)

			l_tool_bar_content := docking_manager.tool_bar_manager.content_by_title (interface_names.m_project_toolbar)
			check not_void: l_tool_bar_content /= Void end
			l_tool_bar_content.set_top ({SD_ENUMERATION}.top)

			l_tool_bar_content := docking_manager.tool_bar_manager.content_by_title (interface_names.m_refactoring_toolbar)
			check not_void: l_tool_bar_content /= Void end
			l_tool_bar_content.set_top ({SD_ENUMERATION}.top)
	...
	restore_editors_docking_layout is
		...
		do
			...
			if l_raw_file.exists then
				l_result := docking_manager.open_tools_config (docking_config_editors_file)
			end
		end
...
feature -- Multiple editor management
	...
	update_paste_cmd is
		...
		do
			if
				not uis.current_editor.is_empty and then
				uis.current_editor.is_editable and then
				uis.current_editor.clipboard.has_text
			then
...
feature {EB_FEATURES_TOOL, EB_FEATURES_TREE, DOTNET_CLASS_AS,
			 DOTNET_CLASS_CONTEXT, EB_STONE_CHECKER, EB_DEVELOPMENT_WINDOW_PART} -- Feature Clauses
...
feature {EB_DEVELOPMENT_WINDOW_PART, EB_STONE_FIRST_CHECKER, EB_DEVELOPMENT_WINDOW_BUILDER} -- Implementation
	...
	locate_editor_stoned (a_stone: STONE) is
			-- Select the editor with `a_stone'
		require
			a_stone_attached: a_stone /= Void
		local
			l_editor: EB_SMART_EDITOR
		do
			l_editor ?= a_stone.pos_editor
			if l_editor = Void then
				l_editor := editors_manager.editor_with_stone (a_stone)
			end
			if l_editor = Void then
				editors_manager.create_editor
				l_editor := editors_manager.last_created_editor
			end
			editors_manager.editor_switched_actions.block
			editors_manager.select_editor (l_editor)
			editors_manager.editor_switched_actions.resume
		end
...
feature {EB_DEVELOPMENT_WINDOW_MENU_BUILDER, EB_DEVELOPMENT_WINDOW_PART,
		...
		do
			if a_feature /= Void then
				...
				if l_efeature /= Void then
					...
					tools.features_tool.seek_ast_item_in_feature_tool (a_feature.feature_names.first.internal_name)
				end
			else
	...
	select_all is
		...
		local
			l_editor: EB_EDITOR
		do
			l_editor := uis.current_editor
			if l_editor /= Void and then not l_editor.is_recycled and then not l_editor.is_empty then
				l_editor.select_all
			end
		end
	...
	goto is
		...
		do
			ed ?= uis.current_editor
			if ed /= Void then
				create l_dialog.make (ed)
				uis.set_goto_dialog (l_dialog)
	...
	cut_selection is
		...
		do
			uis.current_editor.cut_selection
		end

	copy_selection is
			-- Cut the selection in the current editor.
		do
			uis.current_editor.copy_selection
		end

	toggle_formatting_marks is
			-- Show/Hide formatting marks in the editor and update related menu item.
		do
			uis.current_editor.toggle_view_invisible_symbols
			if uis.current_editor.view_invisible_symbols then
...
feature {EB_DEVELOPMENT_WINDOW_DIRECTOR, EB_DEVELOPMENT_WINDOW_BUILDER, EB_ADDRESS_MANAGER} --EB_DEVELOPMENT_WINDOW_DIRECTOR issues.
	...
	set_internal_development_window_data (a_data: like internal_development_window_data) is
			-- Set `internal_development_window_data'
		do
			internal_development_window_data := a_data
		ensure
			set: internal_development_window_data = a_data
		end
