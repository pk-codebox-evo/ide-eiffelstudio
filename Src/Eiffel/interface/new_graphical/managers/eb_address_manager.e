...
feature {NONE} -- Initialization
	...
	build_interface is
		...
		do
			...
			-- FIXIT: why for context tool, still build this?
			if mode then
				hbox.extend (label)
				hbox.disable_item_expand (label)
			else
	...
	on_new_tab_command is
		...
		do
			if class_address.is_displayed and class_address.is_sensitive then
				create l_env
				l_env.application.idle_actions.extend_kamikaze (agent class_address.set_focus)
			end
		end
...
feature {NONE} -- Implementation of the clickable labels for `header_info'
	...
	generate_header_info is
		...
		do
			...
			header_info.set_padding (2)
			header_info.extend (cluster_label)
			header_info.extend (class_label)
			header_info.extend (feature_label)
			header_info.disable_item_expand (cluster_label)
			header_info.disable_item_expand (class_label)
			header_info.disable_item_expand (feature_label)
		end
