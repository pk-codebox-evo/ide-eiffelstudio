...
feature -- Access
	...
	editor_with_stone (a_stone: STONE) : like current_editor is
		...
		do
			if a_stone /= Void then
				...
				loop
					if a_stone = l_editors.item.stone then
	...
	open_classes: HASH_TABLE [FILE_NAME, INTEGER] is
			-- Open classes.
		local
		...
		do
			...
			loop
				...
				if l_classc_stone /= Void then
					Result.put (l_classc_stone.file_name, editor_number_factory.editor_number_from_title (l_editors.item.docking_content.unique_title))
				end
				l_editors.forth
			end
		end

	open_clusters: HASH_TABLE [STRING, INTEGER] is
			-- Open clusters.
		local
		...
		do
			...
			loop
				...
				if l_cluster_stone /= Void then
					Result.put (l_cluster_stone.cluster_i.cluster_name, editor_number_factory.editor_number_from_title (l_editors.item.docking_content.unique_title))
				end
				l_editors.forth
			end
		end
...
feature -- Status report
	...
	is_class_editing (a_file_name: FILE_NAME) : BOOLEAN is
			-- Editor editing a class with `a_file_name'
		require
			a_file_name_attached: a_file_name /= Void
		local
...
feature -- Element change
	...
	restore_editors (a_open_classes: HASH_TABLE [FILE_NAME, INTEGER]; a_open_clusters: HASH_TABLE [STRING, INTEGER]) is
			-- Restore editors.
		require
		...
		local
			...
			l_fack_editors: ARRAYED_LIST [EB_SMART_EDITOR]
		do
			if not a_open_classes.is_empty or not a_open_clusters.is_empty then
				...
				loop
					...
					if l_classi /= Void then
						l_content := create_docking_content_fack_one (editor_number_factory.editor_name (a_open_classes.key_for_iteration), l_content)

						l_fack_editors.extend (last_created_editor)

						l_content.set_long_title (l_classi.name_in_upper)
						l_content.set_short_title (l_classi.name_in_upper)
						l_content.set_pixmap (pixmap_from_class_i (l_classi))
						l_editor_numbers.extend (a_open_classes.key_for_iteration)
				...
				loop
					...
					if l_cluster /= Void then
						...
						init_editor
						l_content := create_docking_content (editor_number_factory.editor_name (a_open_clusters.key_for_iteration), last_created_editor, exist_content)

						l_editor_numbers.extend (a_open_clusters.key_for_iteration)
						last_created_editor.set_docking_content (l_content)
						l_content.set_focus
				...
				show_editors_possible (l_fack_editors)
			end
		ensure
			not_opening_editors: not is_opening_editors
		end

	show_editors_possible (l_fack_editors: ARRAYED_LIST [EB_SMART_EDITOR]) is
			-- Show editors which are possible to show.
		require
			not_void: l_fack_editors /= Void
		do
			from
				l_fack_editors.start
			until
				l_fack_editors.after
			loop
				if l_fack_editors.item.docking_content.user_widget.is_displayed then
					on_focus (l_fack_editors.item)
				end
				l_fack_editors.forth
			end
		end
...
feature {NONE}-- Implementation
	...
	on_focus (a_editor: like current_editor) is
		...
		local
			l_fack_editor: EB_FAKE_SMART_EDITOR
			l_editor: EB_SMART_EDITOR
		do
			if not is_opening_editors then
				l_fack_editor ?= a_editor
				if l_fack_editor /= Void then
					init_editor
					change_fake_to_real (last_created_editor, l_fack_editor, l_fack_editor.content)
	...
	create_docking_content_fack_one (a_unique_title: STRING; a_content: SD_CONTENT): SD_CONTENT is
	...
	change_fake_to_real (a_editor, a_fack_editor: like current_editor; a_content: SD_CONTENT) is
		...
		do
			...
			a_editor.set_stone (a_fack_editor.stone)
		end
