...
feature -- Supplier checking
	...
	check_one_supplier (cl_name: STRING; a_light_supplier: BOOLEAN) is
		...
		do
			...
			if supplier_class /= Void and then not (cl_name.is_equal ("NONE") or cl_name.is_equal (super_none_name)) then
			...
			elseif a_light_supplier then
				if not (cl_name.is_equal ("NONE") or cl_name.is_equal (super_none_name)) then
				...
				if not cl_name.is_equal (super_none_name) then
...
feature {NONE} -- Implementation
	...
	super_none_name: STRING is
			-- Class name of super-none
		do
