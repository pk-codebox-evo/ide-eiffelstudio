		...
		do
		    ...
		    if not has_found then
    		    ...
    		    if has_found then
    		        ...
    		        create l_exp_list.make (l_as.invariant_part.count + 2)
    		        l_exp_list.force (l_as.stop)
    		        l_exp_list.fill (l_as.invariant_part)
    		        l_exp_list.force (l_as.variant_part)
    		        ...
    		        if has_found then
    		        	if loop_depth = 1  then
    		        	    ...
    		        	    l_complete_collector.config (context_class, exception_position, l_as)
