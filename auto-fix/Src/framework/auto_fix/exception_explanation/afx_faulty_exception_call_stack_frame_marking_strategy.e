	...
	mark (an_exception: EQA_TEST_INVOCATION_EXCEPTION; a_frame_list: DS_LINEAR [AFX_EXCEPTION_CALL_STACK_FRAME_I])
		...
		do
		    ...
		    if not l_exception_frames.is_empty then
		        ...
		        l_class := l_exception_frames.last.e_feature.associated_class
		        ...
		        loop
		            ...
		            l_cur_class := l_frame.e_feature.associated_class
			...
			if l_count_relevant_frames > 0 then
			    l_logging_service.log (
			    		l_entry_factory.make_info_entry (
			    				Msg_finished_marking_relevant_exception_frames
			    						+ l_count_relevant_frames.out
			    						+ Msg_number_relevant_exception_frames_found))
			else
			    l_logging_service.log (
			    		l_entry_factory.make_error_entry (
			    				Err_no_relevant_exception_frame_found))
			end
		end
...
feature{NONE} -- Implementation
	...
	eliminate_rescued_frames (a_frame_list: DS_LINEAR [AFX_EXCEPTION_CALL_STACK_FRAME_I])
		...
		do
		    ...
		    l_logging_service.log (
		    		l_entry_factory.make_info_entry (
		    				(l_count - internal_frame_list.count).out + Msg_number_exception_frame_rescue_removed_post))
