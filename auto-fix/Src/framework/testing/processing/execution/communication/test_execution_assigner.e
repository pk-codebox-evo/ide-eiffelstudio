...
feature {TEST_EVALUATOR_STATUS} -- Query
	...
	next_test: like test_count
		...
		do
			...
			io.put_string ("assigner.next_test: " + Result.out)
			io.put_new_line
			cursor_mutex.unlock
		ensure
...
feature -- Status report
	...
	is_aborted (a_test_id: like test_count): BOOLEAN
			...
			--
			-- `a_test_id': Index of test for which we want to know whether it has been aborted.
...
feature {TEST_EXECUTOR_I} -- Status setting
	...
	set_aborted (a_test_id: like test_count)
			...
			--
			-- `a_test_id': Index of test that has been aborted.
		require
			a_test_id_valid: is_valid_test_index (a_test_id)
		do
			cursor_mutex.lock
