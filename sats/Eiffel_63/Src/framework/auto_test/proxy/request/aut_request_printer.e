...
feature -- Access
	...
	last_request: TUPLE [flag: NATURAL_8; test_case_count: INTEGER; execution_flag: NATURAL_16; data: ANY]
			-- Last request
...
feature {AUT_REQUEST} -- Processing
	...
	process_start_request (a_request: AUT_START_REQUEST) is
		do
			last_request := [start_request_flag, a_request.test_case_index, a_request.execution_flag, ""]
		end

	process_stop_request (a_request: AUT_STOP_REQUEST) is
		do
			last_request := [quit_request_flag,  a_request.test_case_index, a_request.execution_flag, ""]
		end
	...
	process_create_object_request (a_request: AUT_CREATE_OBJECT_REQUEST) is
		...
		do
			...
			print_execute_request (l_compound, a_request.test_case_index, a_request.execution_flag)
		end
	...
	process_type_request (a_request: AUT_TYPE_REQUEST) is
		do
			last_request := [type_request_flag, a_request.test_case_index, a_request.execution_flag, a_request.variable.index.out]
		end
...
feature {NONE} -- Byte code generation
	...
	print_execute_request (a_compound: BYTE_LIST [BYTE_NODE]; a_test_case_count: INTEGER; a_execution_flag: NATURAL_16) is
		...
		do
			...
			last_request := [execute_request_flag, a_test_case_count, a_execution_flag, l_byte_code_data]
		end
