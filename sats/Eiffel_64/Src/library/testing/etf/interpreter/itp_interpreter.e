...
--							execute_protected
	...
	record_queries (o: ANY) is
		...
		do
			...
			elseif attached {ACTIVE_LIST [detachable ANY]} o as l_active_list then
				record_query_active_list (l_active_list)
			elseif attached {ARRAY [detachable ANY]} o as l_array then
				record_query_array (l_array)
			elseif attached {ARRAYED_SET [detachable ANY]} o as l_arrayed_set then
				record_query_arrayed_set (l_arrayed_set)
			elseif attached {BINARY_SEARCH_TREE [detachable COMPARABLE]} o as l_binary_search_tree then
				record_query_binary_search_tree (l_binary_search_tree)
			elseif attached {HASH_TABLE [detachable ANY, detachable HASHABLE]} o as l_hash_table then
				record_query_hash_table (l_hash_table)
			elseif attached {HEAP_PRIORITY_QUEUE [detachable COMPARABLE]} o as l_heap_priority_queue then
				record_query_heap_priority_queue (l_heap_priority_queue)
			end
		end
		...
		record_query_active_list (o: ACTIVE_LIST [detachable ANY]) is
				-- Record queries from `o'.
				-- Note: This is a walkaround for the issue that the interpreter
				-- cannot deal with exceptions in melted code correctly.
				-- Remove this feature when that issue is fixed. 19.03.2009 Jason
		...
		record_query_array (o: ARRAY [detachable ANY]) is
				-- Record queries from `o'.
				-- Note: This is a walkaround for the issue that the interpreter
				-- cannot deal with exceptions in melted code correctly.
				-- Remove this feature when that issue is fixed. 19.03.2009 Jason
			...
			do
					-- Please only add argument-less queries of return type BOOLEAN or INTEGER_32.
					-- The queries should be added in the name ascending order.
				record_query (agent o.additional_space)
				record_query (agent o.all_cleared)
				record_query (agent o.all_default)
				record_query (agent o.capacity)
				record_query (agent o.changeable_comparison_criterion)
				record_query (agent o.count)
				record_query (agent o.empty)
				record_query (agent o.extendible)
				record_query (agent o.full)
				record_query (agent o.Growth_percentage)
				record_query (agent o.is_empty)
				record_query (agent o.lower)
				record_query (agent o.Minimal_increase)
				record_query (agent o.object_comparison)
				record_query (agent o.prunable)
				record_query (agent o.resizable)
				record_query (agent o.upper)
				record_query (agent o.valid_index_set)
			end

		record_query_arrayed_set (o: ARRAYED_SET [detachable ANY]) is
				-- Record queries from `o'.
				-- Note: This is a walkaround for the issue that the interpreter
				-- cannot deal with exceptions in melted code correctly.
				-- Remove this feature when that issue is fixed. 19.03.2009 Jason
		...
		record_query_binary_search_tree (o: BINARY_SEARCH_TREE [detachable COMPARABLE]) is
				-- Record queries from `o'.
				-- Note: This is a walkaround for the issue that the interpreter
				-- cannot deal with exceptions in melted code correctly.
				-- Remove this feature when that issue is fixed. 19.03.2009 Jason
			...
			do
					-- Please only add argument-less queries of return type BOOLEAN or INTEGER_32.
					-- The queries should be added in the name ascending order.
				record_query (agent o.arity)
				record_query (agent o.changeable_comparison_criterion)
				record_query (agent o.child_after)
				record_query (agent o.child_before)
				record_query (agent o.Child_capacity)
				record_query (agent o.child_index)
				record_query (agent o.child_isfirst)
				record_query (agent o.child_islast)
				record_query (agent o.child_off)
				record_query (agent o.child_readable)
				record_query (agent o.child_writable)
				record_query (agent o.count)
				record_query (agent o.empty)
				record_query (agent o.has_both)
				record_query (agent o.has_left)
				record_query (agent o.has_none)
				record_query (agent o.has_right)
				record_query (agent o.is_empty)
				record_query (agent o.is_leaf)
				record_query (agent o.is_root)
				record_query (agent o.object_comparison)
				record_query (agent o.Readable)
				record_query (agent o.readable_child)
				record_query (agent o.sorted)
				record_query (agent o.Writable)
				record_query (agent o.writable_child)
			end

		record_query_hash_table (o: HASH_TABLE [detachable ANY, detachable HASHABLE]) is
				-- Record queries from `o'.
				-- Note: This is a walkaround for the issue that the interpreter
				-- cannot deal with exceptions in melted code correctly.
				-- Remove this feature when that issue is fixed. 19.03.2009 Jason
			...
			do
				...
				record_query (agent o.capacity)
				record_query (agent o.changeable_comparison_criterion)
				record_query (agent o.conflict)
				record_query (agent o.count)
				record_query (agent o.empty)
				record_query (agent o.Extendible)
				record_query (agent o.found)
				record_query (agent o.Full)
				record_query (agent o.inserted)
				record_query (agent o.is_empty)
				record_query (agent o.not_found)
				record_query (agent o.object_comparison)
				record_query (agent o.off)
				record_query (agent o.prunable)
				record_query (agent o.removed)
				record_query (agent o.replaced)
			end

		record_query_heap_priority_queue (o: HEAP_PRIORITY_QUEUE [detachable COMPARABLE]) is
				-- Record queries from `o'.
				-- Note: This is a walkaround for the issue that the interpreter
				-- cannot deal with exceptions in melted code correctly.
				-- Remove this feature when that issue is fixed. 19.03.2009 Jason
