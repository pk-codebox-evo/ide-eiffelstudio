...
feature -- status report
	...
	is_resolved: BOOLEAN
			-- <Precursor>
		do
		    Result := is_information_complete and then e_feature /= Void and then breakpoint_info /= Void
		end
...
feature{NONE} -- Implementation
		...
		do
			...
			l_logging_service.log (
						l_entry_factory.make_info_entry (
									Msg_finished_resolving_exception_frame_e_feature_pre + class_name + "." + routine_name))
		ensure
	...
	resolve_breakpoint_info
		...
		do
			...
			l_logging_service.log (
						l_entry_factory.make_info_entry (
									Msg_finished_resolving_exception_frame_e_feature_pre + routine_name + "%A" + breakpoint_slot_index.out))
		ensure
