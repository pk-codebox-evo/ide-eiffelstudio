...
feature {NONE} -- Implementation
	...
	process_n_way_branch(br:TUPLE[AST_EIFFEL]) is
...
feature -- Roundtrip
	...
	process_inline_agent_creation_as (l_as: INLINE_AGENT_CREATION_AS)
			-- Process `l_as'.
		do
			process_n_way_branch([l_as.body, l_as.operands])
		end

	process_custom_attribute_as (l_as: CUSTOM_ATTRIBUTE_AS)
		do
			process_n_way_branch([l_as.creation_expr, l_as.tuple])
		end

	process_static_access_as (l_as: STATIC_ACCESS_AS)
		do
			process_n_way_branch([l_as.class_type, l_as.internal_parameters])
		end

	process_feature_clause_as (l_as: FEATURE_CLAUSE_AS)
		do
			process_n_way_branch([l_as.clients, l_as.features])
		end

	process_tuple_as (l_as: TUPLE_AS)
		do
			process_n_way_branch([l_as.expressions])
		end
	...
	process_body_as (l_as: BODY_AS)
		do
			process_n_way_branch([l_as.arguments, l_as.type, l_as.assigner, l_as.content])
		end

	process_access_feat_as (l_as: ACCESS_FEAT_AS)
		do
			process_n_way_branch([l_as.internal_parameters])
		end

	process_precursor_as (l_as: PRECURSOR_AS)
		do
			process_n_way_branch([l_as.parent_base_class, l_as.internal_parameters])
		end

	process_nested_expr_as (l_as: NESTED_EXPR_AS)
		do
			process_n_way_branch([l_as.target, l_as.message])
		end
	...
	process_creation_expr_as (l_as: CREATION_EXPR_AS)
		do
			process_n_way_branch([l_as.type, l_as.call])
		end

	process_type_expr_as (l_as: TYPE_EXPR_AS)
		do
			process_n_way_branch([l_as.type])
		end

	process_routine_as (l_as: ROUTINE_AS)
		do
			process_n_way_branch([l_as.precondition, l_as.locals, l_as.routine_body, l_as.postcondition, l_as.rescue_clause])
		end

	process_constant_as (l_as: CONSTANT_AS)
		do
			process_n_way_branch([l_as.value])
		end
	...
	process_operand_as (l_as: OPERAND_AS)
		do
			process_n_way_branch([l_as.class_type, l_as.expression, l_as.target])
		end

	process_tagged_as (l_as: TAGGED_AS)
		do
			process_n_way_branch([l_as.expr])
		end
	...
	process_expr_call_as (l_as: EXPR_CALL_AS)
		do
			process_n_way_branch([l_as.call])
		end
	...
	process_routine_creation_as (l_as: ROUTINE_CREATION_AS)
		do
			process_n_way_branch([l_as.target, l_as.feature_name, l_as.operands])
		end
	...
	process_binary_as (l_as: BINARY_AS)
		do
			process_n_way_branch([l_as.left, l_as.right])
		end

	process_bracket_as (l_as: BRACKET_AS)
		do
			process_n_way_branch([l_as.target, l_as.operands])
		end

	process_object_test_as (l_as: OBJECT_TEST_AS)
		do
			if l_as.is_attached_keyword then
				process_n_way_branch([l_as.type, l_as.expression, l_as.name])
			else
				process_n_way_branch([l_as.name, l_as.type, l_as.expression])
			end
		end

	process_feature_as (l_as: FEATURE_AS)
		do
			process_n_way_branch([l_as.feature_names, l_as.body, l_as.indexes])
		end

	process_feature_list_as (l_as: FEATURE_LIST_AS)
		do
			process_n_way_branch([l_as.features])
		end

	process_assign_as (l_as: ASSIGN_AS)
		do
			process_n_way_branch([l_as.target, l_as.source])
		end
	...
	process_check_as (l_as: CHECK_AS)
		do
			process_n_way_branch([l_as.check_list])
		end

	process_creation_as (l_as: CREATION_AS)
		do
			process_n_way_branch([l_as.target, l_as.type, l_as.call])
		end

	process_if_as (l_as: IF_AS)
		do
			process_n_way_branch([l_as.condition, l_as.compound, l_as.elsif_list, l_as.else_part])
		end

	process_inspect_as (l_as: INSPECT_AS)
		do
			process_n_way_branch([l_as.switch, l_as.case_list, l_as.else_part])
		end
	...
	process_loop_as (l_as: LOOP_AS)
		do
			process_n_way_branch([l_as.from_part, l_as.invariant_part, l_as.stop, l_as.compound, l_as.variant_part])
		end

	process_attribute_as (l_as: ATTRIBUTE_AS)
		do
			process_n_way_branch([l_as.compound])
		end
	...
	process_class_as (l_as: CLASS_AS)
		do
			process_n_way_branch ([l_as.top_indexes, l_as.class_name, l_as.generics, l_as.parents, l_as.creators, l_as.convertors, l_as.features, l_as.invariant_part, l_as.bottom_indexes])
		end

	process_parent_as (l_as: PARENT_AS)
		do
			process_n_way_branch([l_as.type, l_as.renaming, l_as.exports, l_as.undefining, l_as.redefining, l_as.selecting])
		end

	process_formal_dec_as (l_as: FORMAL_DEC_AS)
		do
			process_n_way_branch([l_as.constraints, l_as.creation_feature_list])
		end

	process_constraining_type_as (l_as: CONSTRAINING_TYPE_AS)
		do
			process_n_way_branch([l_as.type, l_as.renaming])
		end

	process_class_type_as (l_as: CLASS_TYPE_AS)
		do
			process_n_way_branch([l_as.class_name])
		end

	process_generic_class_type_as (l_as: GENERIC_CLASS_TYPE_AS)
		do
			process_n_way_branch([l_as.class_name, l_as.internal_generics])
		end

	process_named_tuple_type_as (l_as: NAMED_TUPLE_TYPE_AS)
		do
			process_n_way_branch([l_as.class_name, l_as.parameters])
		end

	process_rename_as (l_as: RENAME_AS)
		do
			process_n_way_branch([l_as.old_name, l_as.new_name])
		end

	process_invariant_as (l_as: INVARIANT_AS)
		do
			process_n_way_branch([l_as.assertion_list])
		end

	process_interval_as (l_as: INTERVAL_AS)
		do
			process_n_way_branch([l_as.lower, l_as.upper])
		end

	process_index_as (l_as: INDEX_AS)
		do
			process_n_way_branch([l_as.index_list])
		end
	...
	process_elseif_as (l_as: ELSIF_AS)
		do
			process_n_way_branch([l_as.expr, l_as.compound])
		end

	process_create_as (l_as: CREATE_AS)
		do
			process_n_way_branch([l_as.clients, l_as.feature_list])
		end

	process_client_as (l_as: CLIENT_AS)
		do
			process_n_way_branch([l_as.clients])
		end

	process_case_as (l_as: CASE_AS)
		do
			process_n_way_branch([l_as.interval, l_as.compound])
		end

	process_ensure_as (l_as: ENSURE_AS)
		do
			process_n_way_branch([l_as.assertions])
		end
	...
	process_parameter_list_as (l_as: PARAMETER_LIST_AS)
		...
		do
			...
			if l_list /= Void and then not l_list.is_empty then
				process_n_way_branch([l_as.parameters])
			end
		end
	...
	process_rename_clause_as (l_as: RENAME_CLAUSE_AS)
			-- Process `l_as'.
		do
			process_n_way_branch([l_as.content])
		end
