...
feature -- Access
	...
	assertion: detachable AFX_EXPRESSION
...
feature -- Basic operations
	...
	rewrite (a_tc: AFX_TEST_CASE_INFO; a_structure: AFX_FEATURE_AST_STRUCTURE_NODE; a_trace: STRING)
		...
		do
			...
			elseif a_tc.exception_code = {EXCEP_CONST}.check_instruction then
...
feature{NONE} -- Implementation
	...
	analyze_precondtion_violation (a_tc: AFX_TEST_CASE_INFO; a_structure: AFX_FEATURE_AST_STRUCTURE_NODE; a_trace: STRING)
		...
		do
			...
			l_failing_feati := first_class_starts_with_name (l_failing_feature.a_class).feature_named (l_failing_feature.a_feature)
			create l_calling_args.make (l_failing_feati.argument_count)
			if l_failing_feati.argument_count > 0 then
				from
	...
	assertion_with_tag (a_assertions: LIST [AUT_EXPRESSION]; a_tag: STRING): AUT_EXPRESSION
		...
		do
			...
			loop
				...
				if attached {TAGGED_AS} l_expression.ast as l_tagged then
					if a_tag ~ once "noname" and then (l_tagged.tag = Void or else l_tagged.tag.name.is_empty) then
						l_done := True
						Result := l_expression
					else
						if l_tagged.tag.name ~ a_tag then
							l_done := True
							Result := l_expression
						end
					end
