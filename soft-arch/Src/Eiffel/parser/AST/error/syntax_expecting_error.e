indexing
	description:
		"Represents a syntax error caused by the absence of an expected term."
	legal: "See notice at end of class."
	status: "See notice at end of class."
	date: "$Date$"
	revision: "$Revision $"

class SYNTAX_EXPECTED_ERROR
...
feature {NONE} -- Initialization
	...
	make (a_line: like line; a_column: like column; a_expected: like expected; a_found: like found; a_file_name: like file_name) is
		...
		local
			...
			l_len: INTEGER
		do
			l_len := a_expected.count + 9
			if a_found /= Void then
				l_len := l_len + a_found.count + 15
			end
			create l_message.make (l_len)
			l_message.append (once "Expected ")
			l_message.append (a_expected)
			if a_found /= Void then
				l_message.append (once " instead of ")
				l_message.append (a_found)
...
invariant
	...
	not_expected_is_empty: not expected.is_empty
