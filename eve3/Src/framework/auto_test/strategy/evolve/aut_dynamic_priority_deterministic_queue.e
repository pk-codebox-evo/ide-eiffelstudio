...
class AUT_DYNAMIC_PRIORITY_DETERMINISTIC_QUEUE
	inherit
...
feature -- Basic routines
	...
	select_next
			...
			-- `last_feature'.
			--REDEFINITION so that it selects a feature according to the evolved chromosome
		local
			i: INTEGER
			list: DS_LINKED_LIST [AUT_FEATURE_OF_TYPE]
		do
	...
	--			current_selected := current_selected + 1
			if parameter_loader.is_evolving_method_call  then
				...
				i := (parameter_loader.get_next_method_call \\ static_feature_list.count) + 1
